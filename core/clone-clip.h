...
// ``words`` times are absolute in the same timeline as ``window_start_seconds``.
// Words whose start falls in [window_start, window_start + requested_duration)
// are candidates. The result may end after the requested duration (up to
...
CloneClipBounds refine_clone_clip_bounds(
    ...
    float max_extension_seconds = 1.5f, float end_pad_seconds = 0.05f);
