...
struct MoonshineTensorView {
  ...
  MoonshineTensorView(const OrtApi *ort_api, OrtValue *ort_tensor, const std::string &name = "");
  ...
  template <typename T> T *data() { return static_cast<T *>(_tensor->data); }
  ...
  // You need to call ort_api->ReleaseValue(output_ort_tensor) to release this OrtValue.
  OrtValue* create_ort_value(const OrtApi *ort_api, OrtMemoryInfo *memory_info);

  std::string to_string();
};

template <> inline void *MoonshineTensorView::data<void>() { return static_cast<void *>(_tensor->data); }
template <> inline float *MoonshineTensorView::data<float>() {
...
template <> inline double *MoonshineTensorView::data<double>() {
...
template <> inline int8_t *MoonshineTensorView::data<int8_t>() {
...
template <> inline int16_t *MoonshineTensorView::data<int16_t>() {
...
template <> inline int32_t *MoonshineTensorView::data<int32_t>() {
...
template <> inline int64_t *MoonshineTensorView::data<int64_t>() {
...
template <> inline uint8_t *MoonshineTensorView::data<uint8_t>() {
...
template <> inline uint16_t *MoonshineTensorView::data<uint16_t>() {
...
template <> inline uint32_t *MoonshineTensorView::data<uint32_t>() {
...
template <> inline uint64_t *MoonshineTensorView::data<uint64_t>() {
