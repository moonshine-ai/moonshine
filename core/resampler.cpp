...
const std::vector<float> downsample_audio(const std::vector<float> &audio,
                                           float input_sample_rate,
                                           float output_sample_rate) {
  const int32_t input_audio_size = audio.size();
  int32_t output_audio_size =
  ...
  for (int32_t i = 0; i < output_audio_size; i++) {
    // Calculate the range of input samples that contribute to this output sample
    float start_pos = i * ratio;
    float end_pos = (i + 1) * ratio;
    
    int32_t start_idx = static_cast<int32_t>(start_pos);
    int32_t end_idx = static_cast<int32_t>(end_pos);
...
const std::vector<float> upsample_audio(const std::vector<float> &audio,
  ...
  for (int32_t i = 0; i < output_audio_size; i++) {
    ...
    int32_t index = static_cast<int32_t>(pos);
