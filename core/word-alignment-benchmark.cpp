...
static BenchResult run_benchmark(const char* model_path, const char* wav_path,
  ...
  int32_t handle = moonshine_load_transcriber_from_files(
      model_path, model_arch, options, opt_count, moonshine_get_version());
  if (handle < 0) {
  ...
  for (int i = 0; i < warmup; i++) {
    ...
    moonshine_transcribe_without_streaming(handle, audio, num_samples, 16000, 0,
                                           &t);
  }
...
int main(int argc, char** argv) {
  ...
  auto r1 =
      run_benchmark(model_path, wav_path, false, warmup, runs, model_arch);
  ...
  printf("  Overhead:                %+.1f ms  (%+.1f%%)\n",
         r2.avg_ms - r1.avg_ms, (r2.avg_ms - r1.avg_ms) / r1.avg_ms * 100.0);
