...
std::vector<std::string> korean_g2p_keys() {
  // Rule-based Korean G2P uses the lexicon only (see ``try_korean``). Morph
  // ONNX is test/tooling-only.
...
std::vector<std::string> french_g2p_keys() {
  return {
      std::string(kG2pFrenchDictKey),    std::string(kG2pFrenchPosAdjKey),
      std::string(kG2pFrenchPosAdvKey),  std::string(kG2pFrenchPosConjKey),
      std::string(kG2pFrenchPosDetKey),  std::string(kG2pFrenchPosNounKey),
      std::string(kG2pFrenchPosPrepKey), std::string(kG2pFrenchPosPronKey),
...
g2p_dependency_map() {
  static const std::unordered_map<std::string, std::vector<std::string>> k =
      [] {
        ...
        const std::vector<std::string> kVi = {
            std::string(kG2pVietnameseDictKey)};
        ...
        auto add = [&m](std::string_view key,
...
std::optional<std::vector<std::string>> lookup_g2p_dependency_keys(
    std::string_view raw) {
  ...
  const std::string alt =
      hyphen_to_underscore(normalize_rule_based_dialect_cli_key(lk));
...
void moonshine_asset_catalog_populate_default_g2p_files(
...
moonshine_asset_catalog_g2p_dependency_keys(std::string_view lang_cli) {
...
moonshine_asset_catalog_all_registered_language_tags() {
  // One canonical BCP47-style tag per supported locale (hyphens). Aliases (e.g.
  // ``ar``, ``ko``) stay in
  // ``g2p_dependency_map`` for client lookups only.
  return std::vector<std::string>{
      "ar-msa", "de-de", "en-gb", "en-us", "es-ar", "es-es",   "es-mx",
      "fr-fr",  "hi-in", "it-it", "ja-jp", "ko-kr", "nl-nl",   "pt-br",
      "pt-pt",  "ru-ru", "tr-tr", "uk-ua", "vi-vn", "zh-hans",
  };
}
