...
/// NFC-style key: lowercase letters + umlauts + ß only (Python
...
bool is_german_word_char(char32_t cp) {
  ...
  if (cp == U'ä' || cp == U'ö' || cp == U'ü' || cp == U'ß' || cp == U'Ä' ||
      cp == U'Ö' || cp == U'Ü') {
    return true;
  }
  // Mirror Python ``text_to_ipa`` tokenization ``[\w\-]+``: letters outside
  // ASCII German still form one token; :func:`normalize_lookup_key` / rules
  // strips them so e.g. Greek glosses become "".
...
std::string final_devoice(std::string ipa) {
  ...
  if (ipa.size() >= 2 &&
      static_cast<unsigned char>(ipa[ipa.size() - 2]) == 0xC9 &&
...
bool st_sp_at_morpheme_start(const std::u32string& hyphen_word,
...
size_t unstressed_prefix_len_u32(const std::u32string& w) {
  static const std::u32string prefs[] = {U"wider", U"entgegen", U"ver", U"zer",
                                         U"miss",  U"ent",      U"emp", U"ge",
                                         U"be",    U"er"};
...
std::vector<std::pair<size_t, size_t>> german_vowel_nucleus_spans(
  ...
  while (i < n) {
    ...
    if (i + 1 < n) {
      ...
      if (pair == U"au" || pair == U"ei" || pair == U"eu" || pair == U"ai" ||
          pair == U"äu" || pair == U"ey" || pair == U"oi") {
...
std::vector<std::u32string> german_orthographic_syllables_u32(
    const std::u32string& word_lower) {
  std::vector<std::u32string> out;
  std::u32string w;
  w.reserve(word_lower.size());
  for (char32_t c : word_lower) {
    if (c == U'-' || is_vowel_l(c) || (c >= U'a' && c <= U'z') || c == U'ä' ||
        c == U'ö' || c == U'ü' || c == U'ß') {
      w.push_back(c);
    }
  }
...
size_t default_stress_syllable_index(const std::vector<std::u32string>& syls,
  ...
  if (ends_with(U"ung") || ends_with(U"schaft") || ends_with(U"tion") ||
      ends_with(U"ismus")) {
    return n - 1;
  }
...
std::string insert_primary_stress_before_vowel_utf8(std::string s) {
  ...
  static const char* kPat[] = {
      "aɪ̯", "aʊ̯", "ɔʏ̯", "iː", "eː", "aː", "oː", "uː", "ɪ", "ʊ", "a",
      "ɛ",  "ə",  "i",  "o",  "ɔ",  "u",  "y",  "ø",  "œ", "ʏ", "ɐ",
  };
...
bool ipa_starts_with_nucleus(std::string_view rest) {
  static const std::string_view kNuc[] = {
      "aɪ̯", "aʊ̯", "ɔʏ̯", "ɛɪ̯", "iː", "eː", "aː", "oː", "uː", "yː",
      "øː", "ŋ̩",  "n̩",  "m̩",  "l̩",  "r̩",  "ə",  "ɛ",  "ɜ",  "ɪ",
      "ʊ",  "ɐ̯",  "ɐ",  "ɨ",  "ɵ",  "ø",  "œ",  "ʏ",  "y",  "ɔ",
      "ɑ",  "æ",  "a",  "i",  "e",  "o",  "u",
  };
...
std::string letters_to_ipa_no_stress(const std::u32string& syl_lower,
  ...
  while (i < n) {
    ...
    if (i + 1 < n && s.substr(i, 2) == U"st" &&
        st_sp_at_morpheme_start(hyphen_word, gi)) {
      append("ʃt");
      i += 2;
      continue;
    }
    if (i + 1 < n && s.substr(i, 2) == U"sp" &&
        st_sp_at_morpheme_start(hyphen_word, gi)) {
      append("ʃp");
      i += 2;
      continue;
    ...
    if (i + 1 < n && (s.substr(i, 2) == U"ei" || s.substr(i, 2) == U"ai" ||
                      s.substr(i, 2) == U"ey")) {
      append("aɪ̯");
      i += 2;
      continue;
    ...
    if (i + 1 < n && s.substr(i, 2) == U"ie" &&
        (i + 2 >= n || s[i + 2] == U'-' || !is_vowel_l(s[i + 2]))) {
      append("iː");
      i += 2;
      continue;
    }
    if (i + 1 < n && is_vowel_l(s[i]) && s[i + 1] == s[i] &&
        (s[i] == U'a' || s[i] == U'o' || s[i] == U'e' || s[i] == U'i' ||
         s[i] == U'u')) {
      switch (s[i]) {
        case U'a':
          append("aː");
  ...
  if (stem.size() >= 2 && stem.substr(stem.size() - 2) == U"ig") {
    const bool lich =
        stem.size() >= 4 && stem.substr(stem.size() - 4) == U"lich";
    if (!lich && out.size() >= 2 &&
        static_cast<unsigned char>(out[out.size() - 2]) == 0xC9 &&
...
std::string rules_word_to_ipa_utf8(const std::string& raw_word,
  ...
  for (char32_t cp : utf8_str_to_u32(raw_word)) {
    const char32_t cl = german_tolower_cp(cp);
    if (cl == U'-' || is_vowel_l(cl) || (cl >= U'a' && cl <= U'z') ||
        cl == U'ä' || cl == U'ö' || cl == U'ü' || cl == U'ß') {
      wl.push_back(cl);
    }
  }
  if (wl.empty()) {
  ...
  const size_t stress_idx = with_stress
                                ? default_stress_syllable_index(syls, wl)
...
void load_german_lexicon_stream(
...
void load_german_lexicon_file(
    const std::filesystem::path& path,
    std::unordered_map<std::string, std::string>& out_map) {
  std::ifstream in(path);
  if (!in) {
    throw std::runtime_error("German lexicon: cannot open " +
                             path.generic_string());
  }
...
std::string german_under_100_word(int n) {
  static const char* kDigitStandalone[] = {"null", "eins", "zwei",  "drei",
                                           "vier", "fünf", "sechs", "sieben",
                                           "acht", "neun"};
  static const char* kUnitCompound[] = {"",     "ein",  "zwei",  "drei",
                                        "vier", "fünf", "sechs", "sieben",
                                        "acht", "neun"};
  static const char* kTens[] = {"",        "",        "zwanzig", "dreißig",
...
std::string german_hundred_head(int h) {
  ...
  static const char* stems[] = {"zwei",  "drei",   "vier", "fünf",
...
std::string expand_cardinal_digits_to_german_words(std::string_view s) {
  ...
  if (s.size() > 1 && s[0] == '0') {
    static const char* dw[] = {"null", "eins",  "zwei",   "drei", "vier",
...
std::string GermanRuleG2p::text_to_ipa_no_expand(
...
std::string GermanRuleG2p::text_to_ipa(std::string text,
