...
/// ONNX BIO segmentation + UPOS + ``data/zh_hans/dict.tsv`` (mirrors
/// ``chinese_rule_g2p.ChineseOnnxLexiconG2p``).
class ChineseOnnxG2p {
 public:
  ...
  ChineseOnnxG2p(const MoonshineG2POptions& opt,
                 std::filesystem::path model_dir,
                 std::filesystem::path dict_tsv, bool use_cuda = false);
  ChineseOnnxG2p(const MoonshineG2POptions& opt,
                 std::filesystem::path model_dir, std::string dict_tsv_utf8,
                 bool use_cuda = false);
...
/// ``MoonshineG2P`` adapter: same dialect ids as ``ChineseRuleG2p`` but
/// requires the RoBERTa UPOS ONNX bundle.
class ChineseOnnxRuleG2p : public RuleBasedG2p {
 public:
  ...
  explicit ChineseOnnxRuleG2p(std::filesystem::path onnx_model_dir,
                              std::string dict_tsv_utf8, bool use_cuda = false);
  ChineseOnnxRuleG2p(const MoonshineG2POptions& opt,
