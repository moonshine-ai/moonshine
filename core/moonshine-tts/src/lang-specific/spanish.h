...
/// Pronunciation preset for rule-based Spanish G2P (mirrors
/// ``spanish_rule_g2p.SpanishDialect``).
...
class SpanishRuleG2p : public RuleBasedG2p {
 public:
  ...
  /// Single surface word (no spaces); punctuation should be stripped by the
  /// caller.
...
SpanishDialect spanish_dialect_from_cli_id(
    const std::string& cli_id, bool narrow_intervocalic_obstruents = true);

/// One word → IPA (letters outside Spanish set stripped; punctuation should be
/// handled by caller).
...
/// Tokenize like Python ``text_to_ipa`` (Spanish letters + digits; collapse
/// spaces). If *per_word_log* is non-null, appends one entry per Spanish word
/// span (rule-based path).
