...
/// Rule- and lexicon-based Dutch G2P, mirroring ``dutch_rule_g2p.py`` /
/// ``dutch_numbers.py``.
class DutchRuleG2p : public RuleBasedG2p {
 public:
  struct Options {
    bool with_stress = true;
    /// When true (default), move ˈ/ˌ before the syllable nucleus for
    /// **rule-based** IPA only.
    bool vocoder_stress = true;
    /// Expand ``\b\d+\b`` and ``\b\d+-\d+\b`` to Dutch cardinal words before
    /// G2P.
  ...
  explicit DutchRuleG2p(std::string dict_tsv_utf8, Options options);
  explicit DutchRuleG2p(std::string dict_tsv_utf8)
      : DutchRuleG2p(std::move(dict_tsv_utf8), Options{}) {}
  ...
  /// Same algorithm as Python ``normalize_ipa_stress_for_vocoder`` (Dutch
  /// nucleus list).
...
/// Default lexicon path: ``<model-root>/../data/nl/dict.tsv`` then
