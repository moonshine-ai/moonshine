  ...
  for (const std::u32string& btok : basic_tokens) {
    ...
    if (cursor + btok.size() > ref.size() ||
      ...
      throw std::runtime_error(
          "Japanese WordPiece: basic token alignment failed at offset " +
...
std::filesystem::path default_japanese_tok_pos_model_dir(
    const std::filesystem::path& g2p_data_root) {
  return g2p_data_root / "ja" / "roberta_japanese_char_luw_upos_onnx";
}

JapaneseTokPosOnnx::JapaneseTokPosOnnx(std::filesystem::path model_dir,
                                       bool use_cuda)
    : JapaneseTokPosOnnx(nullptr, std::string_view{}, std::move(model_dir),
                         use_cuda) {}

JapaneseTokPosOnnx::JapaneseTokPosOnnx(const MoonshineG2POptions* opt,
  ...
  if (!bundle_load_utf8(opt_ptr, onnx_bundle_key, "meta.json", model_dir_,
                        meta_json)) {
    throw std::runtime_error(
        "JapaneseTokPosOnnx: missing meta.json (memory or disk under " +
        model_dir_.string() + ")");
  }
  ...
  if (!bundle_load_utf8(opt_ptr, onnx_bundle_key, "tokenizer_config.json",
    ...
    throw std::runtime_error(
        "JapaneseTokPosOnnx: missing tokenizer_config.json");
  }
...
std::vector<std::pair<std::string, std::string>> JapaneseTokPosOnnx::annotate(
  ...
  while (cp_start < full_u32.size()) {
    ...
    if (num_labels != static_cast<int64_t>(id2label_.size())) {
      throw std::runtime_error(
          "JapaneseTokPosOnnx: logits last dim != id2label size");
    }
