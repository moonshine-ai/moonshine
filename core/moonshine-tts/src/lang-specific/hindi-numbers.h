...
/// ASCII digits only → Hindi cardinal words (Devanagari), mirroring
/// ``hindi_numbers.expand_cardinal_digits_to_hindi_words``.
std::string expand_cardinal_digits_to_hindi_words(std::string_view s);

/// Expand ``\\b\\d+-\\d+\\b`` and ``\\b\\d+\\b`` in *text* (ASCII digit
/// tokens).
