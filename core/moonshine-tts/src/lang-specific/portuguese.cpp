...
void load_pt_lexicon_stream(std::istream& in,
...
void load_pt_lexicon_file(const std::filesystem::path& path,
                          std::unordered_map<std::string, std::string>& out) {
  std::ifstream in(path);
  if (!in) {
    throw std::runtime_error("Portuguese G2P: cannot read lexicon " +
                             path.generic_string());
  }
...
static const char* kDigPt[10] = {"zero",   "um",    "dois", "tr\xc3\xaas",
                                 "quatro", "cinco", "seis", "sete",
                                 "oito",   "nove"};

static const char* kTensPt[10] = {
    "",          "",         "vinte",   "trinta",  "quarenta",
    "cinquenta", "sessenta", "setenta", "oitenta", "noventa"};

static const char* kHundredsPt[10] = {
...
void below_1_000_000_tokens_pt(int n, bool is_pt_pt,
...
std::string expand_cardinal_digits_to_portuguese_words(std::string_view s,
                                                       bool is_pt_pt) {
...
bool is_pt_word_char(char32_t cp) {
  ...
  return cp == U'à' || cp == U'á' || cp == U'â' || cp == U'ã' || cp == U'ç' ||
         cp == U'é' || cp == U'ê' || cp == U'í' || cp == U'ó' || cp == U'ô' ||
         cp == U'õ' || cp == U'ú' || cp == U'ü' || cp == U'ý';
}
...
PortugueseRuleG2p::PortugueseRuleG2p(std::filesystem::path dict_tsv,
                                     bool is_portugal)
...
std::string PortugueseRuleG2p::finalize_ipa(std::string ipa,
...
std::string PortugueseRuleG2p::lookup_or_rules(
    const std::string& raw_word) const {
  const std::string letters_only =
      portuguese_rules::normalize_lookup_key_utf8(raw_word);
  if (letters_only.empty()) {
    return "";
  }
  const auto rom =
      portuguese_rules::roman_numeral_token_to_ipa(letters_only, is_portugal_);
  if (rom.has_value()) {
  ...
  const auto& fw =
      is_portugal_ ? portuguese_rules::fw_pt() : portuguese_rules::fw_br();
  auto fi = fw.find(letters_only);
  if (fi != fw.end()) {
    return finalize_ipa(fi->second, false);
  }
  std::string ipa_rules = portuguese_rules::rules_word_to_ipa_utf8(
      trim_ascii_ws_copy(raw_word), is_portugal_, options_.with_stress);
  const bool oov_sc = portuguese_rules::sc_straddle().count(letters_only) != 0;
  if (is_portugal_ && options_.apply_pt_pt_final_esh && !oov_sc) {
    ipa_rules = portuguese_rules::pt_pt_apply_rules_final_s_to_esh(
        std::move(ipa_rules), letters_only);
  }
...
std::string PortugueseRuleG2p::word_to_ipa(const std::string& word) const {
  ...
  if (options_.expand_cardinal_digits && is_all_ascii_digits(wraw)) {
    const std::string phrase =
        expand_cardinal_digits_to_portuguese_words(wraw, is_portugal_);
    if (phrase != wraw) {
...
std::string PortugueseRuleG2p::text_to_ipa_no_expand(
  ...
  while (pos < n) {
    ...
    while (pos < n) {
      utf8_decode_at(text, pos, cp, adv);
      size_t tmp = 0;
      if (try_consume_pt_word(text, pos, tmp) || cp == U' ' || cp == U'\t' ||
          cp == U'\n' || cp == U'\r') {
        break;
      }
      pos += adv;
...
std::string PortugueseRuleG2p::text_to_ipa(
...
bool dialect_resolves_to_portugal_rules(std::string_view dialect_id) {
  ...
  return s == "pt-pt" || s == "portugal" || s == "european-portuguese" ||
         s == "europeanportuguese";
}

bool dialect_resolves_to_brazilian_portuguese_rules(
    std::string_view dialect_id) {
  const std::string s = normalize_rule_based_dialect_cli_key(dialect_id);
  if (s.empty()) {
    return false;
  }
  return s == "pt-br" || s == "brazil" || s == "brazilian-portuguese" ||
         s == "brazilianportuguese" || s == "portuguese-brazil";
}

std::vector<std::string> PortugueseRuleG2p::dialect_ids() {
  return dedupe_dialect_ids_preserve_first(
      {"pt-PT", "pt_PT", "portugal", "european-portuguese",
       "europeanportuguese", "pt-BR", "pt_BR", "brazil", "brazilian-portuguese",
       "brazilianportuguese", "portuguese-brazil"});
}

std::filesystem::path resolve_portuguese_dict_path(
    const std::filesystem::path& model_root, bool is_portugal) {
