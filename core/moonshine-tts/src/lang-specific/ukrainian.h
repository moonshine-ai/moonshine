...
/// Rule-based Ukrainian G2P (mirrors ``ukrainian_rule_g2p.py``): Cyrillic +
/// palatalization lookahead, apostrophe blocking, **г/ґ**, **в** allophony,
/// penultimate stress heuristic, optional digit expansion.
class UkrainianRuleG2p : public RuleBasedG2p {
 public:
  ...
  bool expand_cardinal_digits() const {
    return options_.expand_cardinal_digits;
  }
...
std::string ukrainian_word_to_ipa(const std::string& word,
                                  bool with_stress = true,
                                  bool expand_cardinal_digits = true);

std::string ukrainian_text_to_ipa(
    const std::string& text, bool with_stress = true,
