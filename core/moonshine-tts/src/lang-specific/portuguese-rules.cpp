...
bool is_pt_key_cp(char32_t c) {
  ...
  return c == U'à' || c == U'á' || c == U'â' || c == U'ã' || c == U'ç' ||
         c == U'é' || c == U'ê' || c == U'í' || c == U'ó' || c == U'ô' ||
         c == U'õ' || c == U'ú' || c == U'ü' || c == U'ý';
}
...
bool is_vowel_pt_u32(char32_t ch) {
  ch = pt_tolower(ch);
  return ch == U'a' || ch == U'e' || ch == U'i' || ch == U'o' || ch == U'u' ||
         ch == U'à' || ch == U'á' || ch == U'â' || ch == U'ã' || ch == U'é' ||
         ch == U'ê' || ch == U'í' || ch == U'ó' || ch == U'ô' || ch == U'õ' ||
         ch == U'ú' || ch == U'ü' || ch == U'ý';
}
...
bool should_hiatus_pt_u32(char32_t a, char32_t b) {
  ...
  if (al == U'í' || al == U'ú' || al == U'ý' || bl == U'í' || bl == U'ú' ||
      bl == U'ý') {
    return true;
  }
  ...
  if (al == U'ã' || al == U'õ' || bl == U'ã' || bl == U'õ') {
    if (ba <= U'u' && bb <= U'u' && ba >= U'a' && bb >= U'a' &&
        (ba == U'a' || ba == U'e' || ba == U'o') &&
  ...
  if (sa && sb) {
    if (al == U'á' || al == U'é' || al == U'ó' || al == U'â' || al == U'ê' ||
        al == U'ô' || bl == U'á' || bl == U'é' || bl == U'ó' || bl == U'â' ||
        bl == U'ê' || bl == U'ô') {
      return true;
    }
    if (ba == U'a' && bb == U'e') {
...
std::vector<std::pair<size_t, size_t>> vowel_nucleus_spans_u32(
...
bool valid_onset2_end_u32(char32_t a, char32_t b) {
  static const std::u32string k2[] = {U"bl", U"br", U"cl", U"cr", U"dr",
...
std::vector<std::u32string> port_orthographic_syllables_u32(
...
bool accented_syllable_u32(const std::u32string& s) {
  for (char32_t c : s) {
    if (c == U'á' || c == U'à' || c == U'â' || c == U'é' || c == U'ê' ||
        c == U'í' || c == U'ó' || c == U'ô' || c == U'ú') {
      return true;
    }
  }
  return false;
...
size_t default_stressed_syllable_index_u32(
    const std::vector<std::u32string>& syls, const std::u32string& w) {
  if (syls.empty()) {
    return 0;
  }
  ...
  if (last == U's' && ln >= 2) {
    ...
    if (prev == U'a' || prev == U'e' || prev == U'i' || prev == U'o' ||
        prev == U'u' || prev == U'á' || prev == U'é' || prev == U'í' ||
        prev == U'ó' || prev == U'ú' || prev == U'ã' || prev == U'õ' ||
        prev == U'â' || prev == U'ê' || prev == U'ô') {
      return (n >= 2) ? n - 2 : 0;
    }
  }
  if (last == U'a' || last == U'e' || last == U'o' || last == U'á' ||
      last == U'é' || last == U'ó') {
    return (n >= 2) ? n - 2 : 0;
  }
...
std::string insert_primary_stress_before_vowel_utf8(std::string ipa) {
  ...
  for (size_t i = 0; i < u.size(); ++i) {
    ...
    if (ch == U'a' || ch == U'e' || ch == U'i' || ch == U'o' || ch == U'u' ||
        ch == U'\u025B' || ch == U'\u0254' || ch == U'\u0250' ||
        ch == U'\u026A' || ch == U'\u028A' || ch == U'\u0268' ||
        ch == U'\u0259' || ch == U'\u00E6' || ch == U'\u0254') {
      std::string pre;
...
std::optional<int> roman_to_int_ascii(std::string_view u) {
  ...
  const std::unordered_map<char, int> vals{{'I', 1},   {'V', 5},   {'X', 10},
                                           {'L', 50},  {'C', 100}, {'D', 500},
                                           {'M', 1000}};
  int total = 0;
  size_t i = 0;
...
std::optional<std::string> roman_numeral_token_to_ipa(
    const std::string& letters_lower, bool is_pt_pt) {
  if (letters_lower.find('-') != std::string::npos ||
      letters_lower.find('\'') != std::string::npos) {
    return std::nullopt;
  }
...
bool next_global_vowel_u32(const std::u32string& full_word, size_t gidx) {
  // Mirrors Python ``_next_global_vowel``: ``j = gidx + 1`` (first index
  // examined is after *gidx*).
  size_t j = gidx + 1;
...
std::string letters_to_ipa_no_stress_u32(const std::u32string& s, bool is_pt_pt,
                                         const std::u32string& full_word,
                                         size_t span_start,
                                         bool stressed_syllable) {
  ...
  while (i < n) {
    ...
    if (s[i] == U'c' && i > 0 && s[i - 1] == U's' && i + 1 < n) {
      ...
      if (v == U'a' || v == U'á' || v == U'â' || v == U'e' || v == U'é' ||
          v == U'ê' || v == U'i' || v == U'í' || v == U'o' || v == U'ó' ||
          v == U'ô' || v == U'u' || v == U'ú' || v == U'ã' || v == U'õ') {
    ...
    if (s[i] == U'r') {
      ...
      const bool intervocal = gidx > 0 && is_vowel_pt_u32(prev_ch) &&
                              gidx + 1 < full_word.size() &&
    ...
    if (is_vowel_pt_u32(ch)) {
      if (i + 1 < n && is_vowel_pt_u32(s[i + 1]) &&
          !should_hiatus_pt_u32(ch, s[i + 1])) {
        const char32_t a = pt_tolower(ch);
        const char32_t b = pt_tolower(s[i + 1]);
        if ((a == U'a' || a == U'à' || a == U'á' || a == U'â') &&
            (b == U'i' || b == U'í')) {
          out += "aj";
          i += 2;
          continue;
...
std::string rules_word_to_ipa_single_u32(const std::u32string& wl,
                                         bool is_pt_pt, bool with_stress) {
  ...
  const int stress_idx =
      with_stress
          ? static_cast<int>(default_stressed_syllable_index_u32(syls, wl))
          : -1;
  size_t offset = 0;
  std::string parts;
  for (size_t idx = 0; idx < syls.size(); ++idx) {
    std::string chunk = letters_to_ipa_no_stress_u32(
        syls[idx], is_pt_pt, wl, offset,
...
static const std::unordered_set<std::string> kPtFinalSExclude = {
    "\x61\x6e\xc3\xad\x73",     "\x62\xc3\xb4\x6e\x75\x73",
    "\x63\x61\x69\x73",         "\x63\x61\x6f\x73",
    "\x63\x6f\x73\x6d\x6f\x73", "\x66\x72\x61\x6e\x63\xc3\xaa\x73",
    "\x66\xc3\xa9\x6e\x69\x78", "\x69\x6e\x67\x6c\xc3\xaa\x73",
    "\x6c\xc3\xa1\x70\x69\x73", "\x6d\xc3\xaa\x73",
    "\x70\x61\xc3\xad\x73",     "\x70\x6f\x72\x74\x75\x67\x75\xc3\xaa\x73",
    "\x74\x72\xc3\xaa\x73",     "\x74\xc3\xb3\x72\x61\x78",
...
bool vowel_grapheme_tail_pt(char32_t c) {
  ...
  return c == U'a' || c == U'e' || c == U'i' || c == U'o' || c == U'u' ||
         c == U'à' || c == U'á' || c == U'â' || c == U'ã' || c == U'é' ||
         c == U'ê' || c == U'í' || c == U'ó' || c == U'ô' || c == U'õ' ||
         c == U'ú' || c == U'ü';
}

std::string pt_pt_apply_rules_final_s_to_esh(std::string ipa,
                                             const std::string& letters_key) {
  if (ipa.empty()) {
    return ipa;
  }
  ...
  if (ends_u32(lku, kEs) || ends_u32(lku, kAs) || ends_u32(lku, kIs) ||
      ends_u32(lku, kUs)) {
    return ipa;
  }
...
const std::unordered_map<std::string, std::string>& fw_br_map_impl() {
  static const std::unordered_map<std::string, std::string> kFwBr = {
      ...
      {"e", "i"},
      {"ou", "ow"},
      {"em", "\xc9\x90\xcc\x83\x6a\xcc\x83"},
      {"no", "n\xca\x8a"},
      {"na", "n\xc9\x90"},
      {"nos", "n\xca\x8a\x73"},
      {"nas", "n\xc9\x90\x73"},
      {"de", "d\xca\x92\xc9\xaa"},
      {"do", "d\xca\x8a"},
      {"da", "d\xc9\x90"},
      {"dos", "d\xca\x8a\x73"},
      {"das", "d\xc9\x90\x73"},
      {"dum", "d\xc5\xa9"},
      {"duma", "\xcb\x88\x64\x75\x6d\xc9\x90"},
      {"num", "n\xc5\xa9"},
      {"numa", "\xcb\x88\x6e\x75\x6d\xc9\x90"},
      {"pelo", "\xcb\x88\x70\xc9\x9b\x6c\xca\x8a"},
      {"pela", "\xcb\x88\x70\xc9\x9b\x6c\xc9\x90"},
      {"pelos", "\xcb\x88\x70\xc9\x9b\x6c\xca\x8a\x73"},
      {"pelas", "\xcb\x88\x70\xc9\x9b\x6c\xc9\x90\x73"},
      {"com", "k\xc3\xb5"},
      {"sem", "s\xc9\x90\xcc\x83\x6a\xcc\x83"},
      {"por", "p\x6f\xc9\xbe"},
      {"para", "\xcb\x88\x70\x61\xc9\xbe\xc9\x90"},
      {"que", "k\x69"},
      {"n\xc3\xa3o", "\xcb\x88\x6e\xc9\x90\xcc\x83\x77\xcc\x83"},
      {"um", "\xc5\xa9"},
      {"uma", "\xcb\x88\x75\x6d\xc9\x90"},
      {"uns", "\xc5\xa9\x73"},
      {"umas", "\xcb\x88\x75\x6d\xc9\x90\x73"},
      {"ao", "aw"},
      {"aos", "aw\xca\x83"},
      {"\xc3\xa0", "a"},
      {"\xc3\xa0s", "\xc9\x90\xca\x83"},
  };
  return kFwBr;
}
...
const std::unordered_map<std::string, std::string>& fw_pt_map_impl() {
  static const std::unordered_map<std::string, std::string> kFwPt = {
      ...
      {"na", "n\xc9\x90"},
      {"nos", "nu\xca\x83"},
      {"nas", "n\xc9\x90\xca\x83"},
      {"de", "d\xc9\xa8"},
      {"do", "du"},
      {"da", "d\xc9\x90"},
      {"dos", "du\xca\x83"},
      {"das", "d\xc9\x90\xca\x83"},
      {"dum", "d\xc5\xa9"},
      {"duma", "\xcb\x88\x64\x75\x6d\xc9\x90"},
      {"num", "n\xc5\xa9"},
      {"numa", "\xcb\x88\x6e\x75\x6d\xc9\x90"},
      {"pelo", "\xcb\x88\x70\xc9\x9b\x6c\x75"},
      {"pela", "\xcb\x88\x70\xc9\x9b\x6c\xc9\x90"},
      {"pelos", "\xcb\x88\x70\xc9\x9b\x6c\x75\xca\x83"},
      {"pelas", "\xcb\x88\x70\xc9\x9b\x6c\xc9\x90\xca\x83"},
      {"com", "k\xc3\xb5"},
      {"sem", "s\xc9\x90\xcc\x83\x6a\xcc\x83"},
      {"por", "p\x75\xc9\xbe"},
      {"para", "\xcb\x88\x70\xc9\x90\xc9\xbe\xc9\x90"},
      {"que", "k\xc9\xa8"},
      {"n\xc3\xa3o", "\xcb\x88\x6e\xc9\x90\xcc\x83\x77\xcc\x83"},
      {"um", "\xc5\xa9"},
      {"uma", "\xcb\x88\x75\x6d\xc9\x90"},
      {"uns", "\xc5\xa9\xca\x83"},
      {"umas", "\xcb\x88\x75\x6d\xc9\x90\xca\x83"},
      {"ao", "aw"},
      {"aos", "aw\xca\x83"},
      {"\xc3\xa0", "a"},
      {"\xc3\xa0s", "a\xca\x83"},
...
std::string rules_word_to_ipa_utf8(const std::string& raw, bool is_pt_pt,
                                   bool with_stress) {
  ...
  if (wl.find(U'-') != std::u32string::npos) {
    ...
    if (chunks.size() > 1) {
      ...
      for (size_t c = 0; c < chunks.size(); ++c) {
        ...
        joined +=
            rules_word_to_ipa_single_u32(chunks[c], is_pt_pt, with_stress);
      }
      return joined;
    }
  }
