...
/// Rule- and lexicon-based Italian G2P, mirroring ``italian_rule_g2p.py`` /
/// ``italian_numbers.py``.
class ItalianRuleG2p : public RuleBasedG2p {
 public:
  struct Options {
    ...
    /// Expand ``\b\d+\b`` and ``\b\d+-\d+\b`` to Italian cardinal words before
    /// G2P.
  ...
  explicit ItalianRuleG2p(std::string dict_tsv_utf8, Options options);
  explicit ItalianRuleG2p(std::string dict_tsv_utf8)
      : ItalianRuleG2p(std::move(dict_tsv_utf8), Options{}) {}
...
/// Default lexicon: ``<model-root>/../data/it/dict.tsv`` then
/// ``<model-root>/it/dict.tsv``.
