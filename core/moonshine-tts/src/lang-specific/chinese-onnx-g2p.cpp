...
ChineseOnnxG2p::ChineseOnnxG2p(const MoonshineG2POptions& opt,
                               std::filesystem::path model_dir,
                               std::filesystem::path dict_tsv, bool use_cuda)
    : tok_(&opt, kG2pChineseOnnxDirKey, std::move(model_dir), use_cuda),
      lex_(std::move(dict_tsv)) {}
...
std::string ChineseOnnxG2p::text_to_ipa(std::string text_utf8,
  ...
  for (const auto& pr : pairs) {
    ...
    if (per_word_log != nullptr) {
      per_word_log->push_back(
          G2pWordLog{w, w, G2pWordPath::kRuleBasedG2p, wipa});
    }
    if (!wipa.empty()) {
...
ChineseOnnxRuleG2p::ChineseOnnxRuleG2p(std::filesystem::path onnx_model_dir,
                                       ...
                                       bool use_cuda)
    : impl_(std::make_unique<ChineseOnnxG2p>(std::move(onnx_model_dir),
                                             std::move(dict_tsv), use_cuda)) {}

ChineseOnnxRuleG2p::ChineseOnnxRuleG2p(std::filesystem::path onnx_model_dir,
                                       std::string dict_tsv_utf8, bool use_cuda)
    : impl_(std::make_unique<ChineseOnnxG2p>(
          std::move(onnx_model_dir), std::move(dict_tsv_utf8), use_cuda)) {}

ChineseOnnxRuleG2p::ChineseOnnxRuleG2p(const MoonshineG2POptions& opt,
                                       std::filesystem::path onnx_model_dir,
                                       std::filesystem::path dict_tsv,
                                       bool use_cuda)
    : impl_(std::make_unique<ChineseOnnxG2p>(opt, std::move(onnx_model_dir),
                                             std::move(dict_tsv), use_cuda)) {}
...
ChineseOnnxRuleG2p::ChineseOnnxRuleG2p(const MoonshineG2POptions& opt,
                                       ...
                                       std::string dict_tsv_utf8, bool use_cuda)
    : impl_(std::make_unique<ChineseOnnxG2p>(
          opt, std::move(onnx_model_dir), std::move(dict_tsv_utf8), use_cuda)) {
}

std::string ChineseOnnxRuleG2p::text_to_ipa(
