  ...
  for (const std::u32string& btok : basic_tokens) {
    ...
    if (cursor + btok.size() > ref.size() ||
      ...
      throw std::runtime_error(
          "Korean WordPiece: basic token alignment failed at offset " +
...
const std::unordered_set<std::string>& ud_upos_set() {
...
std::string morph_label_to_upos(std::string label) {
  while (!label.empty() &&
         (label.rfind("B-", 0) == 0 || label.rfind("I-", 0) == 0)) {
  ...
  while (start < label.size()) {
    ...
    const std::string part = (plus == std::string::npos)
                                 ? label.substr(start)
                                 : label.substr(start, plus - start);
    if (ud_upos_set().count(part) != 0) {
      return part;
    }
...
std::filesystem::path default_korean_tok_pos_model_dir(
    const std::filesystem::path& g2p_data_root) {
  return g2p_data_root / "ko" / "roberta_korean_morph_upos_onnx";
}

KoreanTokPosOnnx::KoreanTokPosOnnx(std::filesystem::path model_dir,
                                   bool use_cuda)
    : KoreanTokPosOnnx(nullptr, std::string_view{}, std::move(model_dir),
                       use_cuda) {}

KoreanTokPosOnnx::KoreanTokPosOnnx(const MoonshineG2POptions* opt,
  ...
  if (!bundle_load_utf8(opt_ptr, onnx_bundle_key, "meta.json", model_dir_,
                        meta_json)) {
    throw std::runtime_error(
        "KoreanTokPosOnnx: missing meta.json (memory or disk under " +
        model_dir_.string() + ")");
  }
  ...
  if (!bundle_load_utf8(opt_ptr, onnx_bundle_key, "vocab.txt", model_dir_,
                        cached_vocab_txt_)) {
    throw std::runtime_error("KoreanTokPosOnnx: missing vocab.txt");
  }
  if (!bundle_load_utf8(opt_ptr, onnx_bundle_key, "tokenizer_config.json",
                        model_dir_, cached_tokenizer_cfg_json_)) {
    throw std::runtime_error("KoreanTokPosOnnx: missing tokenizer_config.json");
  }
  if (bundle_load_binary(opt_ptr, onnx_bundle_key, onnx_name, model_dir_,
    ...
    if (!std::filesystem::is_regular_file(onnx_path)) {
      throw std::runtime_error("KoreanTokPosOnnx: missing " +
                               onnx_path.string());
    }
...
std::vector<std::pair<std::string, std::string>> KoreanTokPosOnnx::annotate(
  ...
  if (cached_vocab_txt_.empty() || cached_tokenizer_cfg_json_.empty()) {
    throw std::runtime_error("KoreanTokPosOnnx: tokenizer assets not loaded");
  }
  const std::unordered_map<std::string, int64_t> vocab =
  ...
  while (cp_start < full_u32.size()) {
    ...
    if (num_labels != static_cast<int64_t>(id2label_.size())) {
      throw std::runtime_error(
          "KoreanTokPosOnnx: logits last dim != id2label size");
    }
    ...
    for (const std::vector<int>& g : enc.word_groups) {
      ...
      for (int ti : g) {
        ...
        for (int64_t j = 0; j < num_labels; ++j) {
          pooled[static_cast<size_t>(j)] +=
              static_cast<double>(logits[base + static_cast<size_t>(j)]);
        }
      }
      ...
      std::u32string surf_u = enc.ref_u32.substr(
          static_cast<std::size_t>(st), static_cast<std::size_t>(en - st));
