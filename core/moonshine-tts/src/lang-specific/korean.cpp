...
std::string strip_mn_after_nfd(const std::string& ipa) {
  ...
  std::free(nfd);
  // Decode with the same lenient UTF-8 scan as the rest of rule-G2P (lexicon
  // IPA may be messy).
  ...
  for (char32_t cp : u32) {
    if (utf8proc_category(static_cast<utf8proc_int32_t>(cp)) ==
      ...
      continue;  // strip all combining marks (incl. U+031A unreleased, U+0348
                 // tense)
    }
...
bool is_sonorant_jong(int jong) {
  ...
  if (jong >= 8 && jong <= 16)
    return true;                         // ㄹ/ㄹ-clusters (8-15) and ㅁ (16)
...
std::string ipa_onset(int cho, bool tense, bool aspirate) {
  ...
  switch (cho) {
    case kChoK:
      ip = "\xC9\xA1";  // ɡ — lenis ㄱ always voiced (Piper convention for
                        // Korean ㄱ)
      break;
    case kChoKk:
    ...
    case kChoM:
      ...
      break;
    case kChoP:
      ip = "p";  // lenis ㅂ default voiceless (word-initial); voiced b in
    ...
    case kChoSs:
      ...
      break;
    case kChoC:
      ip = "t\xC9\x95";  // tɕ — lenis ㅈ default voiceless (Piper word-initial
                         // convention)
      break;
    case kChoCc:
  ...
  if (aspirate) {
    // Aspiration from ㅎ+lenis → voiceless aspirated (override the voiced
    // defaults)
    if (cho == kChoK) return "kh";
    if (cho == kChoT) return "th";
    if (cho == kChoP) return "ph";
    if (cho == kChoC)
      return "t\xCA\x83h";  // tʃh for aspirated ㅊ (postalveolar, eSpeak
                            // convention)
  }
  return ip;
}
...
std::string ipa_nucleus(int jung) {
  static const char* const vmap[] = {
      "a",         "\xC9\x9B",  "ja",        "j\xC9\x9B", "\xCA\x8C",  "e",
      "j\xCA\x8C", "je",        "o",         "wa",        "w\xC9\x9B", "we",
      "jo",        "u",         "w\xCA\x8C", "we",        "wi",        "ju",
      "\xC9\xAF",  "\xC9\xAFj", "i",
  };
  if (jung < 0 || jung > 20) {
    return "ə";
...
std::string ipa_coda_simple(int jong) {
  ...
  if (jong == 1 || jong == 2 || jong == 3 || jong == 24) {
    return "q";  // ㄱ-type unreleased coda (Piper ko_KR vocoder uses q for
                 // this)
  }
  if (jong == 7 || jong == 25 || jong == 19 || jong == 20 || jong == 22 ||
      jong == 23 || jong == 27) {
...
std::string coda_nasal_assimilate(int jong, std::optional<int> next_cho) {
  if (!next_cho.has_value() ||
      (*next_cho != kChoM && *next_cho != kChoN && *next_cho != kChoNgOnset)) {
  ...
  if (jong == 7 || jong == 19 || jong == 20 || jong == 22 || jong == 23 ||
      jong == 25 || jong == 27 || jong == 12 || jong == 13 || jong == 14 ||
      jong == 15) {
    return "n";
  }
...
std::string syllables_to_ipa(const std::vector<Syllable>& syls,
                             std::string_view syllable_sep) {
  if (syls.empty()) {
    return "";
  }
  ...
  for (std::size_t i = 0; i < syls.size(); ++i) {
    ...
    if (cho != kIdxO) {
      ...
      const bool tense_after = prev != nullptr &&
                               jong_triggers_tense(prev->jong) &&
                               (cho == kChoK || cho == kChoT || cho == kChoP ||
                                cho == kChoS || cho == kChoC);
      if (after_h) {
    ...
    // ㅏ (jung==0) → ɐ (U+0250) in all positions (Piper ko_KR vocoder
    // convention).
    const std::string nucleus =
        (s.jung == 0) ? "\xC9\x90" : ipa_nucleus(s.jung);

    // Embed stress marker immediately before the nucleus vowel (eSpeak-ng Piper
    // convention). Primary stress ˈ (U+02C8) on syllable 0; secondary stress ˌ
    // (U+02CC) on syllables 2,4,6…
    std::string stress;
    if (i == 0) {
      stress = "\xCB\x88";  // ˈ primary
    } else if (i % 2 == 0 && syls.size() >= 3) {
      stress = "\xCB\x8C";  // ˌ secondary (even syllables in words with 3+
                            // syllables)
    }

    std::string coda_ipa;
    if (s.jong != 0) {
      const bool h_lost_before_asp = nxt != nullptr && s.jong == 27 &&
                                     (nxt->cho == kChoK || nxt->cho == kChoT ||
                                      nxt->cho == kChoP || nxt->cho == kChoC);
    ...
    // Piper convention: stress marker sits immediately before the vowel
    // nucleus, AFTER any glide (w/j). So "ˈwa" becomes "wˈa", "ˈjʌ" becomes
    // "jˈʌ".
...
std::vector<std::string> sino_cardinal_speech_units(std::uint64_t n) {
  ...
  if (q > 0)
    units.push_back(
        int_to_sino_korean_hangul(static_cast<std::uint64_t>(q) * 1000ULL));
  if (b > 0)
    units.push_back(
        int_to_sino_korean_hangul(static_cast<std::uint64_t>(b) * 100ULL));
  if (r2 > 0)
    units.push_back(int_to_sino_korean_hangul(static_cast<std::uint64_t>(r2)));
  return units;
}

std::string g2p_hangul_rules_only_inner(std::string_view hangul,
                                        std::string_view syllable_sep) {
  if (hangul.empty()) {
    return "";
  }
...
std::string KoreanRuleG2p::normalize_korean_ipa(std::string ipa,
  ...
  // Preserve tense (fortis) consonants BEFORE strip_mn removes their diacritic
  // (U+0348 ͈). After stripping, remaining plain k/t/p are lenis → will be
  // voiced to ɡ/d/b at the end. Tense ㄲ(k͈) ㄸ(t͈) ㅃ(p͈) ㅆ(s͈) stay voiceless;
  // sentinels protect them through the pipeline.
  std::string pre = ipa;
  const std::string tense_dia =
      "\xCD\x88";  // U+0348 COMBINING DOUBLE VERTICAL LINE BELOW
  ...
  const std::string unrel_dia =
      "\xCC\x9A";  // U+031A COMBINING LEFT ANGLE ABOVE (unreleased)
  ...
  replace_all(t, "t" + tie + "\xC9\x95\xCA\xB0",
              "t\xCA\x83h");  // t͡ɕʰ → tʃh (aspirated → postalveolar)
  replace_all(t, "d" + tie + "\xCA\x91\xCA\xB0", "t\xCA\x83h");  // d͡ʑʰ → tʃh
  replace_all(t, "t" + tie + "\xCA\x83", "t\xCA\x83");           // t͡ʃ  → tʃ
  replace_all(t, "t" + tie + "\xC9\x95",
              "t\xC9\x95");  // t͡ɕ  → tɕ (keep alveolo-palatal)
  replace_all(t, "d" + tie + "\xCA\x91",
              "d\xCA\x91");                      // d͡ʑ  → dʑ (keep voiced)
  ...
  // After strip_mn_after_nfd strips the tie in t͡ɕʰ → tɕʰ, the tɕ prefix must be
  // consumed together with ʰ, otherwise the standalone ɕʰ rule below leaves an
  // orphan leading 't'.
  replace_all(
      t, "t\xC9\x95\xCA\xB0",
      "t\xCA\x83h");  // tɕʰ (tie-stripped) → tʃh  (must precede ɕʰ rule)
  replace_all(t, "\xC9\x95\xCA\xB0",
              "t\xCA\x83h");  // ɕʰ → tʃh (standalone, e.g. from lexicon)
  ...
  replace_all(t, "t\xC9\x95h",
              "t\xCA\x83h");  // tɕh → tʃh (bare aspirated affricate)
  replace_all(t, "\xC9\xAD", "\xC9\xAB");  // ɭ → ɫ
  replace_all(t, "\xC9\xB0",
              "\xC9\xAF");  // ɰ → ɯ (velar approximant not in vocoder map)
  ...
  replace_all(t, "\xCA\x9D",
              "j");  // ʝ → j (voiced palatal fricative not in Piper inventory)
  replace_all(t, "\xC9\xB8",
              "h");  // ɸ → h (voiceless bilabial fricative not in vocoder)
  replace_all(t, "\xC3\xB8",
              "we");  // ø → we (front rounded vowel not in Piper inventory)
  replace_all(t, "\xC9\x98",
              "\xCA\x8C");  // ɘ → ʌ (close-mid central → open-mid back)
  replace_all(
      t, "c",
      "t\xC9\x95");  // plain 'c' → tɕ (palatal stop not in Piper inventory)
  // Strip U+FFFD replacement characters that can arise from malformed lexicon
  // normalization.
  ...
  // Korean ㅏ: Piper ko_KR vocoder uses ɐ (near-open central) rather than a
  // (open front). This must run AFTER all other replacements to avoid mangling
  // multi-byte sequences.
  replace_all(t, "a", "\xC9\x90");  // a → ɐ

  if (voice_lenis) {
    // Voice lenis stops: k→ɡ, p→b, t→d for LEXICON entries only.
    // Rule-based output already has correct voicing from syllables_to_ipa, so
    // this pass is skipped when voice_lenis=false to avoid converting tense
    // k/t/p to voiced. Must protect aspirated kh/ph/th first, then convert,
    // then restore.
...
std::string KoreanRuleG2p::g2p_hangul_rules_only(
    std::string_view hangul) const {
  if (hangul.empty()) {
    return "";
  }
  // voice_lenis=false: rule-based output already has correct voicing from
  // syllables_to_ipa; skip the k→ɡ/p→b/t→d pass that would incorrectly voice
  // tense consonants.
...
void load_korean_lexicon_stream(
  ...
  while (std::getline(in, line)) {
    ...
    if (lexicon.find(k) == lexicon.end()) {
      lexicon.emplace(k,
                      KoreanRuleG2p::normalize_korean_ipa(std::move(ipa_val)));
    }
  }
}
...
KoreanRuleG2p::KoreanRuleG2p(std::filesystem::path dict_tsv, Options options)
  ...
  if (!std::filesystem::is_regular_file(dict_tsv)) {
    throw std::runtime_error("Korean G2P: lexicon not found at " +
                             dict_tsv.generic_string());
  }
  std::ifstream in(dict_tsv);
  if (!in) {
    throw std::runtime_error("Korean G2P: cannot open " +
                             dict_tsv.generic_string());
  }
...
std::string KoreanRuleG2p::text_to_ipa(std::string text,
  ...
  // Ensure IPA has a stress marker positioned after the onset, before the first
  // vowel nucleus (Piper ko_KR vocoder convention: ɡˈɯ not ˈɡɯ or ˈkɯ).
  // Rule-based output already embeds ˈ/ˌ within syllables at the correct
  // position. Lexicon entries may have ˈ at position 0 (before onset) —
  // reposition if so.
  auto add_word_stress = [](std::string ipa) -> std::string {
    ...
    // If stress is already present but NOT at position 0, it's already
    // correctly positioned (e.g. from rule-based output like "dˈɛhɐn").
    ...
    for (size_t p = 0; p < s.size();) {
      ...
      if (c >= 0xC0 && c < 0xE0 && p + 1 < s.size()) {
        ...
        if ((c == 0xC9 &&
             (c2 == 0x90 || c2 == 0x9B || c2 == 0xAF || c2 == 0x94)) ||
            (c == 0xCA && c2 == 0x8C)) {
          s.insert(p, "\xCB\x88");
          return s;
        }
  ...
  // Pre-tokenize: replace brackets/parens with spaces to prevent adjacent
  // Korean words from merging. e.g. "파동함수(켓)에" → "파동함수 켓 에",
  // "(大韓)은" → " 大韓 은".
  ...
  auto log_mapping = [&](const std::string& grapheme, const std::string& ipa,
                         const char* path) {
    if (do_log) {
  ...
  while (iss >> w) {
    if (options_.expand_cardinal_digits) {
      if (const auto frags =
              korean_reading_fragments_from_ascii_numeral_token(w)) {
    ...
    // Handle mixed numeric+Hangul tokens such as "1986년", "7월", "3일",
    // "8개월". Split into natural speech units: 1986 → ["천","구백","팔십육"] +
    // "년" on last unit.
    if (options_.expand_cardinal_digits) {
      size_t num_end = 0;
      ...
      if (has_digit && num_end < w.size()) {
        const std::string hangul_tail =
            extract_hangul(std::string_view(w).substr(num_end));
        if (!hangul_tail.empty()) {
          ...
          if (const auto num_frags =
                  korean_reading_fragments_from_ascii_numeral_token(num_sv)) {
            std::string combined;
            ...
            if (!ipa.empty()) {
              const std::string stressed = add_word_stress(ipa);
              log_mapping(w + " -> " + combined, stressed,
                          "mixed-num+hangul-fallback");
              ipa_words.push_back(stressed);
            }
            continue;
          }
