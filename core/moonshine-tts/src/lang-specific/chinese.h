...
/// Simplified Chinese lexicon G2P (``data/zh_hans/dict.tsv`` ipa-dict IPA),
/// mirroring
/// ``chinese_rule_g2p.ChineseOnnxLexiconG2p`` **without** ONNX segmentation:
/// ``text_to_ipa`` splits on whitespace; **unbroken Han substrings** are one
/// lookup (then per-character fallback). Optional **CTB POS** on
/// ``word_to_ipa_with_pos`` disambiguates common polyphones (行/了/没/…).
/// Arabic numerals expand to Mandarin cardinals like the Python layer.
class ChineseRuleG2p : public RuleBasedG2p {
 public:
  ...
  /// Same as Python ``g2p_word`` with no POS (first lexicon reading when
  /// ambiguous).
  std::string word_to_ipa(std::string_view word) const;

  /// Pass CTB-style or UD UPOS tag (e.g. ``NN``, ``NOUN``, ``VV``, ``VERB``)
  /// for heteronym hints; empty *pos* → default.
  std::string word_to_ipa_with_pos(std::string_view word,
 ...
 private:
  ...
  std::string disambiguate_heteronym(
