...
/// Japanese LUW surfaces + UD UPOS via ONNX
/// (``KoichiYasuoka/roberta-small-japanese-char-luw-upos``), mirroring
/// ``japanese_tok_pos.JapaneseTokPosOnnx`` / ``encode_for_morph_upos`` +
/// WordPiece from
/// ``data/ja/roberta_japanese_char_luw_upos_onnx/``.
class JapaneseTokPosOnnx {
 public:
  explicit JapaneseTokPosOnnx(std::filesystem::path model_dir,
                              bool use_cuda = false);
  JapaneseTokPosOnnx(const MoonshineG2POptions* opt,
                     std::string_view onnx_bundle_key,
                     std::filesystem::path model_dir_fallback,
                     bool use_cuda = false);

  /// One ``(surface, UPOS)`` per whitespace/punctuation word (empty input →
  /// empty list).
  std::vector<std::pair<std::string, std::string>> annotate(
      std::string_view text_utf8);

  /// Same string form as ``japanese_tok_pos`` CLI: ``tok1/UPOS1 tok2/UPOS2 ``
...
/// ``<repo>/data/ja/roberta_japanese_char_luw_upos_onnx`` when *repo_root* is
/// the repository root.
