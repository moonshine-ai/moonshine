...
/// Rule- and lexicon-based German G2P (High German), mirroring
/// ``german_rule_g2p.py``.
class GermanRuleG2p : public RuleBasedG2p {
 public:
  struct Options {
    bool with_stress = true;
    /// When true (default), move ˈ/ˌ before the syllable nucleus
    /// (vocoder-style).
    bool vocoder_stress = true;
    /// Expand ``\\b\\d+\\b`` and year-style ``\\b\\d+-\\d+\\b`` to German
    /// cardinal words before G2P.
    bool expand_cardinal_digits = true;
  };

  /// Load ``word<TAB>IPA`` TSV; throws ``std::runtime_error`` if the file is
  /// missing/unreadable.
  ...
  /// Tokenize like Python ``text_to_ipa`` (German letters, digits, hyphen;
