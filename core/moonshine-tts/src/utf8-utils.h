...
/// Decode one UTF-8 code point starting at byte index *i* in *s*. On success
/// returns true and advances *out_len* bytes (always >= 1). Mirrors the lenient
/// scan used across rule-based G2P.
bool utf8_decode_at(const std::string& s, size_t i, char32_t& out_cp,
                    size_t& out_len);

/// Full-string scan to UTF-32 (invalid sequences yield one code unit per
/// leading byte, same as :func:`utf8_decode_at`).
...
/// Trim ASCII whitespace only (same policy as legacy ``trim_copy_sv`` helpers
/// in language files). IMPORTANT: uses an explicit ASCII-only check —
/// ``std::isspace(0xA0)`` returns true on macOS (Latin-1 non-breaking space),
/// which breaks UTF-8 strings whose last byte happens to be 0xA0 (e.g. Hangul
/// 고 = EA B3 A0, combining marks like U+0320 = CC A0).
...
// Find *token* as a contiguous subsequence of code points in *text*, starting
// at code point index >= start_cp. Returns [start, end) code point indices into
// *text*.
std::optional<std::pair<int, int>> utf8_find_token_codepoints(
    const std::string& text, const std::string& token, int start_cp);

/// Rough Python ``\\w`` neighbor for ``re`` digit spans: ASCII alnum +
/// underscore + major script blocks.
...
/// True if the ASCII digit substring ``text[start_byte:end_byte]`` should
/// expand like Python ``\\b\\d+\\b``.
bool digit_ascii_span_expandable_python_w(const std::string& text,
                                          size_t start_byte, size_t end_byte);

/// Trim ASCII whitespace, map ``_`` → ``-``, and ASCII-lowercase ``A``–``Z``.
/// Used for MoonshineG2P /
/// ``create_rule_based_g2p`` dialect matching (tags are ASCII).
std::string normalize_rule_based_dialect_cli_key(std::string_view raw);

/// Keep the first surface form for each
/// ``normalize_rule_based_dialect_cli_key`` value (drops e.g. both ``pt-PT``
/// and ``pt_PT`` when they normalize to the same key).
