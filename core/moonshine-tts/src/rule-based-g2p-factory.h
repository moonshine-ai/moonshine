...
/// Try to construct a rule-based engine for *dialect_id* (trimmed). Returns
/// nullopt if unknown. Order: English, Spanish, German, French, Dutch, Italian,
/// Russian, Chinese, Korean, Vietnamese, Japanese, Arabic, Portuguese, Turkish,
/// Ukrainian, Hindi.
std::optional<RuleBasedG2pInstance> create_rule_based_g2p(
    std::string_view dialect_id, const MoonshineG2POptions& options);

/// All static ``dialect_ids()`` from each rule engine, in factory resolution
/// order (for discovery).
