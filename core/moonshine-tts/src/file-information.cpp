...
void FileInformation::load(const uint8_t** out_memory, size_t* out_size) {
  ...
  if (path.empty()) {
    throw std::runtime_error(
        "FileInformation::load: empty path and no memory buffer");
  }
  std::ifstream f(path, std::ios::binary);
  if (!f) {
    throw std::runtime_error("FileInformation::load: cannot open " +
                             path.string());
  }
  f.seekg(0, std::ios::end);
  const std::streamoff end = f.tellg();
  if (end < 0) {
    throw std::runtime_error("FileInformation::load: cannot size " +
                             path.string());
  }
  ...
  if (!f) {
    ...
    throw std::runtime_error("FileInformation::load: failed reading " +
                             path.string());
  }
  memory = owned_storage_.data();
  memory_size = owned_storage_.size();
...
void FileInformationMap::set_memory(std::string_view key, const uint8_t* mem,
                                    size_t sz,
                                    std::filesystem::path path_for_resolve) {
  std::filesystem::path p = path_for_resolve.empty()
                                ? std::filesystem::path(std::string(key))
                                : std::move(path_for_resolve);
...
void FileInformationMap::parse_file_list(
  ...
  if (key_list == nullptr) {
    throw std::runtime_error(
        "FileInformationMap::parse_file_list: key_list is null");
  }
  ...
  if (have_mem && (memory_pointers->size() != key_list->size() ||
    ...
    throw std::runtime_error(
        "FileInformationMap::parse_file_list: memory vector sizes must match "
        "key_list");
  }
