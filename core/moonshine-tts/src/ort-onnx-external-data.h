...
/// If ``files`` contains an in-memory ``<stem>.onnx.data`` companion for
/// ``model_map_key`` (either
/// ``…/model.onnx`` or a canonical ``…/model.ort`` key), register it with ORT
/// so
/// ``CreateSessionFromArray`` can resolve external initializers when the
/// process cwd has no data tree.
