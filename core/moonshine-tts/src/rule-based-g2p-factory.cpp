...
std::filesystem::path resolve_french_dict_path(const MoonshineG2POptions& opt) {
  return resolve_path_under_root(opt.g2p_root,
                                 opt.relative_asset_path(kG2pFrenchDictKey));
}

std::filesystem::path resolve_french_csv_dir(const MoonshineG2POptions& opt) {
  return resolve_path_under_root(opt.g2p_root,
                                 opt.relative_asset_path(kG2pFrenchCsvDirKey));
}
...
bool utf8_content_git_lfs_pointer_stub(std::string_view content) {
  ...
  const std::string_view line =
      n == std::string_view::npos ? content : content.substr(0, n);
  static constexpr std::string_view kPrefix =
      "version https://git-lfs.github.com/spec/v1";
  return line.size() >= kPrefix.size() &&
         line.compare(0, kPrefix.size(), kPrefix) == 0;
...
collect_french_pos_csv_from_options(const MoonshineG2POptions& o) {
...
bool g2p_onnx_bundle_reachable(const MoonshineG2POptions& o,
                               std::string_view bundle_dir_key,
...
/// True when ``meta.json`` is available and the model file it names exists on
/// disk or in memory.
bool g2p_onnx_bundle_includes_model_file(
    const MoonshineG2POptions& o, std::string_view bundle_dir_key,
    const std::filesystem::path& disk_dir) {
...
std::optional<RuleBasedG2pInstance> try_english(
    std::string_view trimmed, const MoonshineG2POptions& options) {
  ...
  const std::filesystem::path data_root =
      std::filesystem::is_regular_file(dict_tsv)
          ? dict_tsv.parent_path()
  ...
  if (options.asset_is_available(kG2pEnglishG2pConfigKey)) {
    ...
    if (utf8_content_git_lfs_pointer_stub(cfg)) {
      throw std::runtime_error(
          "English G2P: in-memory g2p-config.json is a Git LFS pointer stub, "
          "not JSON. "
    ...
    if (std::filesystem::is_regular_file(g2p_cfg)) {
      if (file_looks_like_git_lfs_pointer(g2p_cfg)) {
        throw std::runtime_error(
            ...
            " is a Git LFS pointer stub, not JSON. From the moonshine-tts "
            "directory run: git lfs pull");
      }
      std::ifstream cfg_in(g2p_cfg);
      const nlohmann::json j = nlohmann::json::parse(cfg_in);
      if (!j.value("uses_oov_model", true)) {
        throw std::runtime_error("English G2P: " + g2p_cfg.generic_string() +
                                 " must set uses_oov_model true");
      }
    }
  }
  ...
  const auto fill_oov_mem = [&]() {
    const bool ov_reg =
        options.files.entries.find(std::string(kG2pOovOnnxOverrideKey)) !=
    ...
    if (ov_reg) {
      if (options.asset_is_available(kG2pOovOnnxOverrideKey) &&
        ...
        oov_mem = EnglishOnnxAuxMemory{
            options.read_binary_asset(kG2pOovOnnxOverrideKey),
    ...
    if (options.asset_is_available(kG2pEnglishOovModelKey) &&
      ...
      oov_mem = EnglishOnnxAuxMemory{
          options.read_binary_asset(kG2pEnglishOovModelKey),
  ...
  if (!oov_ok) {
    throw std::runtime_error(
        "English G2P: OOV ONNX missing (place model.onnx or model.ort and "
        "onnx-config.json under "
        "en_us/oov/ beneath g2p_root, or register buffers for keys " +
        std::string(kG2pEnglishOovModelKey) + " and " +
        std::string(kG2pEnglishOovOnnxConfigKey) + ")");
  }

  RuleBasedG2pInstance out;
  out.canonical_dialect_id =
      dialect_is_british_english_variant(trimmed) ? "en_gb" : "en_us";
  ...
  if (dict_from_files) {
    ...
    out.engine = std::make_unique<EnglishRuleG2p>(
        std::move(dict_utf8), oov_onnx, options.use_cuda, oov_mem,
        prefer_british, options.ort_provider_names, options.coreml_cache_dir);
  } else {
    out.engine = std::make_unique<EnglishRuleG2p>(
        dict_tsv, oov_onnx, options.use_cuda, oov_mem, prefer_british,
        options.ort_provider_names, options.coreml_cache_dir);
  }
  return out;
}

std::optional<RuleBasedG2pInstance> try_spanish(
    std::string_view trimmed, const MoonshineG2POptions& options) {
  const std::string spanish_key = normalize_spanish_dialect_cli_key(trimmed);
  try {
    SpanishDialect d = spanish_dialect_from_cli_id(
        spanish_key, options.spanish_narrow_obstruents);
    RuleBasedG2pInstance out;
    out.canonical_dialect_id = std::move(d.id);
    out.kind = RuleBasedG2pKind::Spanish;
    out.engine = std::make_unique<SpanishRuleG2p>(std::move(d),
                                                  options.spanish_with_stress);
    return out;
...
std::optional<RuleBasedG2pInstance> try_german(
    std::string_view trimmed, const MoonshineG2POptions& options) {
...
std::optional<RuleBasedG2pInstance> try_french(
    std::string_view trimmed, const MoonshineG2POptions& options) {
  ...
  if (options.asset_is_available(kG2pFrenchDictKey)) {
    ...
    if (!fr_pos_mem.empty()) {
      engine = std::make_unique<FrenchRuleG2p>(
          options.read_utf8_asset(kG2pFrenchDictKey), std::move(fr_pos_mem),
          fo);
    } else {
      engine = std::make_unique<FrenchRuleG2p>(
          options.read_utf8_asset(kG2pFrenchDictKey), fcsv, fo);
    }
    RuleBasedG2pInstance out;
  ...
  std::unique_ptr<FrenchRuleG2p> engine;
  if (!fr_pos_mem.empty()) {
    engine = std::make_unique<FrenchRuleG2p>(read_path_as_utf8(fdict),
...
std::optional<RuleBasedG2pInstance> try_dutch(
    std::string_view trimmed, const MoonshineG2POptions& options) {
  if (!dialect_resolves_to_dutch_rules(trimmed)) {
    return std::nullopt;
  }
  DutchRuleG2p::Options dopts{
      .with_stress = options.dutch_with_stress,
...
std::optional<RuleBasedG2pInstance> try_italian(
    std::string_view trimmed, const MoonshineG2POptions& options) {
  if (!dialect_resolves_to_italian_rules(trimmed)) {
    return std::nullopt;
  }
  ItalianRuleG2p::Options iopts{
      .with_stress = options.italian_with_stress,
...
std::optional<RuleBasedG2pInstance> try_russian(
    std::string_view trimmed, const MoonshineG2POptions& options) {
  if (!dialect_resolves_to_russian_rules(trimmed)) {
    return std::nullopt;
  }
  RussianRuleG2p::Options ropts{
      .with_stress = options.russian_with_stress,
...
std::optional<RuleBasedG2pInstance> try_chinese(
    std::string_view trimmed, const MoonshineG2POptions& options) {
  ...
  if (!g2p_onnx_bundle_includes_model_file(options, kG2pChineseOnnxDirKey,
                                           mdir)) {
    throw std::runtime_error(
        "Chinese G2P: ONNX tokenizer bundle incomplete (need meta.json, "
        "tokenizer files, and the "
        "onnx_model_file named in meta.json under " +
        mdir.generic_string() +
        ", or register matching memory buffers under keys like " +
        std::string(kG2pChineseOnnxMetaKey) +
        "; set MoonshineG2POptions::files / chinese_onnx_model_dir or export "
        "KoichiYasuoka/chinese-roberta-base-upos to "
  ...
  if (options.asset_is_available(kG2pChineseDictKey)) {
    out.engine = std::make_unique<ChineseOnnxRuleG2p>(
        options, mdir, options.read_utf8_asset(kG2pChineseDictKey),
        options.use_cuda);
    return out;
  }
  ...
  out.engine = std::make_unique<ChineseOnnxRuleG2p>(options, mdir, cdict,
                                                    options.use_cuda);
  return out;
}

std::optional<RuleBasedG2pInstance> try_korean(
    std::string_view trimmed, const MoonshineG2POptions& options) {
  ...
  if (options.asset_is_available(kG2pKoreanDictKey)) {
    out.engine = std::make_unique<KoreanRuleG2p>(
        options.read_utf8_asset(kG2pKoreanDictKey), ko);
    return out;
  }
...
std::optional<RuleBasedG2pInstance> try_vietnamese(
    std::string_view trimmed, const MoonshineG2POptions& options) {
  ...
  if (options.asset_is_available(kG2pVietnameseDictKey)) {
    out.engine = std::make_unique<VietnameseRuleG2p>(
        options.read_utf8_asset(kG2pVietnameseDictKey));
    return out;
  }
...
std::optional<RuleBasedG2pInstance> try_japanese(
    std::string_view trimmed, const MoonshineG2POptions& options) {
  ...
  if (!g2p_onnx_bundle_includes_model_file(options, kG2pJapaneseOnnxDirKey,
                                           mdir)) {
    throw std::runtime_error(
        "Japanese G2P: ONNX tokenizer bundle incomplete under " +
        mdir.generic_string() +
        " (need meta.json, tokenizer files, and onnx_model_file; or in-memory "
        "keys under " +
        std::string(kG2pJapaneseOnnxDirKey) +
        "/…); set MoonshineG2POptions::files / japanese_onnx_model_dir or "
        "export the char-LUW model to "
  ...
  if (options.asset_is_available(kG2pJapaneseDictKey)) {
    out.engine = std::make_unique<JapaneseRuleG2p>(
        options, mdir, options.read_utf8_asset(kG2pJapaneseDictKey),
        options.use_cuda);
    return out;
  }
  ...
  out.engine =
      std::make_unique<JapaneseRuleG2p>(options, mdir, jdict, options.use_cuda);
  return out;
}

std::optional<RuleBasedG2pInstance> try_arabic(
    std::string_view trimmed, const MoonshineG2POptions& options) {
  ...
  if (!g2p_onnx_bundle_includes_model_file(options, kG2pArabicOnnxDirKey,
                                           mdir)) {
    throw std::runtime_error(
        "Arabic G2P: ONNX tokenizer bundle incomplete under " +
        mdir.generic_string() +
        " (need meta.json, tokenizer files, and onnx_model_file; or in-memory "
        "keys under " +
  ...
  if (options.asset_is_available(kG2pArabicDictKey)) {
    out.engine = std::make_unique<ArabicRuleG2p>(
        options, mdir, options.read_utf8_asset(kG2pArabicDictKey),
        options.use_cuda);
    return out;
  }
  ...
  out.engine =
      std::make_unique<ArabicRuleG2p>(options, mdir, adict, options.use_cuda);
  return out;
}

std::optional<RuleBasedG2pInstance> try_turkish(
    std::string_view trimmed, const MoonshineG2POptions& options) {
...
std::optional<RuleBasedG2pInstance> try_ukrainian(
    std::string_view trimmed, const MoonshineG2POptions& options) {
...
std::optional<RuleBasedG2pInstance> try_hindi(
    std::string_view trimmed, const MoonshineG2POptions& options) {
  ...
  if (options.asset_is_available(kG2pHindiDictKey)) {
    out.engine = std::make_unique<HindiRuleG2p>(
        options.read_utf8_asset(kG2pHindiDictKey), ho);
    return out;
  }
...
std::optional<RuleBasedG2pInstance> try_portuguese(
    std::string_view trimmed, const MoonshineG2POptions& options) {
  const bool want_pt_br =
      dialect_resolves_to_brazilian_portuguese_rules(trimmed);
  ...
  const bool pt_override_registered =
      options.files.entries.find(pt_override_key) !=
      options.files.entries.end();
  ...
  if (pt_override_registered &&
      options.asset_is_available(kG2pPortugueseDictOverrideKey)) {
    out.engine = std::make_unique<PortugueseRuleG2p>(
        options.read_utf8_asset(kG2pPortugueseDictOverrideKey), is_portugal,
        pto);
    return out;
  }
  if (!pt_override_registered &&
      options.asset_is_available(is_portugal ? kG2pPtPtDictKey
                                             : kG2pPtBrDictKey)) {
    out.engine = std::make_unique<PortugueseRuleG2p>(
        options.read_utf8_asset(is_portugal ? kG2pPtPtDictKey
                                            : kG2pPtBrDictKey),
        is_portugal, pto);
    return out;
  }
  ...
  if (pt_override_registered) {
    if (const auto o =
            options.optional_override_path(kG2pPortugueseDictOverrideKey)) {
      rel_pt = *o;
    } else {
      throw std::runtime_error(
          "Portuguese G2P: portuguese_dict_path map entry has no path and no "
          "in-memory buffer "
          "(set a path or register bytes under that key)");
    }
  } else {
    rel_pt =
        std::filesystem::path(is_portugal ? kG2pPtPtDictKey : kG2pPtBrDictKey);
  }
  const std::filesystem::path pdict =
      resolve_path_under_root(options.g2p_root, rel_pt);
...
using TryFn = std::optional<RuleBasedG2pInstance> (*)(
    std::string_view, const MoonshineG2POptions&);

const TryFn kTryChain[] = {
    try_english,    try_spanish,    try_german,    try_french,
    try_dutch,      try_italian,    try_russian,   try_chinese,
    try_korean,     try_vietnamese, try_japanese,  try_arabic,
    try_portuguese, try_turkish,    try_ukrainian, try_hindi,
};

}  // namespace

std::optional<RuleBasedG2pInstance> create_rule_based_g2p(
    std::string_view dialect_id, const MoonshineG2POptions& options) {
...
std::vector<std::pair<RuleBasedG2pKind, std::vector<std::string>>>
rule_based_g2p_dialect_catalog() {
  ...
  out.emplace_back(RuleBasedG2pKind::Vietnamese,
                   VietnameseRuleG2p::dialect_ids());
  ...
  out.emplace_back(RuleBasedG2pKind::Portuguese,
                   PortugueseRuleG2p::dialect_ids());
  out.emplace_back(RuleBasedG2pKind::Turkish, TurkishRuleG2p::dialect_ids());
  out.emplace_back(RuleBasedG2pKind::Ukrainian,
                   UkrainianRuleG2p::dialect_ids());
