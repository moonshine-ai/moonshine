...
/// ``<bundle_dir_key>/<filename>`` for ``FileInformationMap`` /
/// ``read_*_asset``.
inline std::string g2p_bundle_file_key(std::string_view bundle_dir_key,
...
/// Canonical ``FileInformationMap`` keys for bundled relative assets (default
/// path = key).
...
/// Directory override for French POS CSVs (disk scan); CDN keys use the
/// ``fr/*.csv`` paths below.
...
inline constexpr std::string_view kG2pChineseOnnxDirKey =
    "zh_hans/roberta_chinese_base_upos_onnx";
inline constexpr std::string_view kG2pKoreanDictKey = "ko/dict.tsv";
inline constexpr std::string_view kG2pKoreanOnnxDirKey =
    "ko/roberta_korean_morph_upos_onnx";
...
inline constexpr std::string_view kG2pJapaneseOnnxDirKey =
    "ja/roberta_japanese_char_luw_upos_onnx";
inline constexpr std::string_view kG2pArabicOnnxDirKey =
    "ar_msa/arabertv02_tashkeel_fadel_onnx";
...
inline constexpr std::string_view kG2pEnglishDictKey =
...
inline constexpr std::string_view kG2pPortugueseDictOverrideKey =
    "portuguese_dict_path";
/// Optional English assets (same ``FileInformationMap`` / memory pattern as
/// lexicons).
inline constexpr std::string_view kG2pEnglishG2pConfigKey =
...
inline constexpr std::string_view kG2pOovOnnxConfigOverrideKey =
    "oov_onnx_config";
...
inline constexpr std::string_view kG2pJapaneseOnnxVocabKey =
    "ja/roberta_japanese_char_luw_upos_onnx/vocab.txt";
...
/// Matches ``onnx_model_file`` in bundled ``meta.json`` (``.ort`` in-tree;
/// loaders also accept ``.onnx``).
inline constexpr std::string_view kG2pJapaneseOnnxModelKey =
    "ja/roberta_japanese_char_luw_upos_onnx/model.ort";

inline constexpr std::string_view kG2pKoreanOnnxMetaKey =
    "ko/roberta_korean_morph_upos_onnx/meta.json";
inline constexpr std::string_view kG2pKoreanOnnxVocabKey =
    "ko/roberta_korean_morph_upos_onnx/vocab.txt";
inline constexpr std::string_view kG2pKoreanOnnxTokenizerConfigKey =
    "ko/roberta_korean_morph_upos_onnx/tokenizer_config.json";
/// Default export name; runtime uses ``onnx_model_file`` from ``meta.json``
/// when present.
inline constexpr std::string_view kG2pKoreanOnnxModelKey =
    "ko/roberta_korean_morph_upos_onnx/model.onnx";

inline constexpr std::string_view kG2pArabicOnnxMetaKey =
    "ar_msa/arabertv02_tashkeel_fadel_onnx/meta.json";
inline constexpr std::string_view kG2pArabicOnnxVocabKey =
...
/// Options for constructing ``MoonshineG2P`` (rule-engine paths and toggles;
/// optional English OOV ONNX overrides).
///
/// Lexicon and ONNX paths live in ``files`` under **canonical keys** equal to
/// their default relative paths (e.g. ``fr/dict.tsv``). Set ``g2p_root`` to the
/// directory that contains those subpaths, or leave it empty to resolve those
/// relative paths against the process current working directory only.
struct MoonshineG2POptions {
  ...
  /// Relative path (under ``g2p_root``) for a bundled asset. If ``files`` has
  /// no entry for
  ...
  /// Non-empty path stored under ``map_key`` (used for overrides such as
  /// ``oov_onnx_override``).
  std::optional<std::filesystem::path> optional_override_path(
      std::string_view map_key) const;

  /// True if the asset has a client buffer or exists on disk under ``g2p_root``
  /// (canonical key or map key).
  bool asset_is_available(std::string_view canonical_key) const;

  /// Full file contents via ``FileInformation::load`` (memory buffer or disk).
