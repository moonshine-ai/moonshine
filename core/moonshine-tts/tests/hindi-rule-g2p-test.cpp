...
void check_wiki_parity(const std::filesystem::path& wiki,
  ...
  for (size_t i = 0; i < src.size(); ++i) {
    INFO("wiki line " << (i + 1));
    CHECK(moonshine_tts::hindi_text_to_ipa(src[i], true, nullptr, true,
                                           hi_dict) == py[i]);
  }
}
...
TEST_CASE("hindi: कमल and मैं match reference IPA when golden exists") {
  ...
  const std::filesystem::path golden =
      r::tests_data_dir(repo) / "hi" / "rule_g2p_sample.txt";
  if (!std::filesystem::is_regular_file(golden)) {
    return;
  }
  const auto lines = r::load_ref_lines(golden);
  REQUIRE(lines.size() >= 1);
  const std::filesystem::path hi_dict =
      r::moonshine_tts_bundled_data_dir_relative() / "hi" / "dict.tsv";
  CHECK(moonshine_tts::hindi_text_to_ipa("कमल मैं", true, nullptr, true,
                                         hi_dict) == lines[0]);
}
...
TEST_CASE(
  ...
  const std::filesystem::path wiki =
      r::moonshine_tts_bundled_data_dir_relative() / "hi" / "wiki-text.txt";
  const std::filesystem::path golden =
      r::tests_data_dir(repo) / "hi" / "rule_g2p_wiki_100.txt";
