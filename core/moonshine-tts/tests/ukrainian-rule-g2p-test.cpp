...
void check_wiki_parity(const std::filesystem::path& wiki,
  ...
  for (size_t i = 0; i < src.size(); ++i) {
    INFO("wiki line " << (i + 1));
    CHECK(moonshine_tts::ukrainian_text_to_ipa(src[i]) == py[i]);
  }
}
...
TEST_CASE("ukrainian: m'ясо and кінь match reference IPA when golden exists") {
  ...
  const std::filesystem::path golden =
      r::tests_data_dir(repo) / "uk" / "rule_g2p_sample.txt";
...
TEST_CASE("ukrainian: dialect ids include uk and uk-UA") {
  const auto ids = moonshine_tts::UkrainianRuleG2p::dialect_ids();
...
TEST_CASE(
  ...
  const std::filesystem::path wiki =
      r::moonshine_tts_bundled_data_dir_relative() / "uk" / "wiki-text.txt";
  const std::filesystem::path golden =
      r::tests_data_dir(repo) / "uk" / "rule_g2p_wiki_100.txt";
