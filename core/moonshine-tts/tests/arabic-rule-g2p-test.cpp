...
TEST_CASE(
  ...
  const auto model = data / "ar_msa" / "arabertv02_tashkeel_fadel_onnx";
  const auto dict = data / "ar_msa" / "dict.tsv";
  const auto wiki =
      r::moonshine_tts_bundled_data_dir_relative() / "ar" / "wiki-text.txt";
  const std::filesystem::path golden =
      r::tests_data_dir(repo) / "ar" / "rule_g2p_wiki_100.txt";
  if (!r::model_present(model) || !std::filesystem::is_regular_file(dict) ||
