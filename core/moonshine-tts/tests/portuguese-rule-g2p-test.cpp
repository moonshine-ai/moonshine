...
std::filesystem::path make_temp_tsv(const char* contents) {
  ...
  std::filesystem::path p =
      ...
      ("moonshine_pt_test_" + std::to_string(tick) + ".tsv");
  std::ofstream o(p);
  o << contents;
  return p;
}
...
TEST_CASE("portuguese: casa matches reference IPA when data and golden exist") {
  ...
  const std::filesystem::path dict =
      r::moonshine_tts_bundled_data_dir_relative() / "pt_br" / "dict.tsv";
  const std::filesystem::path golden =
      r::tests_data_dir(repo) / "pt_br" / "rule_g2p_casa.txt";
...
TEST_CASE(
  ...
  const std::filesystem::path wiki =
      r::moonshine_tts_bundled_data_dir_relative() / "pt_br" / "wiki-text.txt";
  const std::filesystem::path golden =
      r::tests_data_dir(repo) / "pt_br" / "rule_g2p_wiki_100.txt";
  if (!std::filesystem::is_regular_file(dict) ||
...
TEST_CASE(
  ...
  const auto repo = r::repo_root_from_tests_cpp(__FILE__);
  const std::filesystem::path dict =
      r::moonshine_tts_bundled_data_dir_relative() / "pt_pt" / "dict.tsv";
  const std::filesystem::path wiki =
      r::moonshine_tts_bundled_data_dir_relative() / "pt_pt" / "wiki-text.txt";
  const std::filesystem::path golden =
      r::tests_data_dir(repo) / "pt_pt" / "rule_g2p_wiki_100.txt";
  if (!std::filesystem::is_regular_file(dict) ||
      !std::filesystem::is_regular_file(wiki) ||
      !std::filesystem::is_regular_file(golden)) {
    return;
