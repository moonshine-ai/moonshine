...
SileroVad::SileroVad(
    ...
    : ort_api(nullptr), env(nullptr), session_options(nullptr), session(nullptr), allocator(nullptr), memory_info(nullptr),
      threshold(threshold), speech_pad_samples(speech_pad_ms)
{
    sr_per_ms = sample_rate / 1000;  // e.g., 16000 / 1000 = 16
    window_size_samples = windows_frame_size * sr_per_ms; // e.g., 32ms * 16 = 512 samples
    effective_window_size = window_size_samples + context_samples; // 512 + 64 = 576 samples
    input_node_dims[0] = 1;
...
int SileroVad::load_from_memory(const uint8_t* model_data, size_t model_data_size) {
    init_onnx_env();
    return ort_session_from_memory(ort_api, env, session_options, model_data, model_data_size, &session);
}
...
// data_chunk is expected to have window_size_samples samples (e.g., 512 for 16kHz).
void SileroVad::predict(const std::vector<float>& data_chunk, float* out_probability, int* out_flag) {
    // Build input by prepending context (64 samples) to data_chunk (512 samples) = 576 total
    input.resize(effective_window_size);
    std::copy(_context.begin(), _context.end(), input.begin());
    std::copy(data_chunk.begin(), data_chunk.end(), input.begin() + context_samples);
    ...
    status = ort_api->CreateTensorWithDataAsOrtValue(memory_info, input.data(), input.size() * sizeof(float), input_node_dims, 2, ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT, &input_ort);
    if (status != nullptr) {
    ...
    status = ort_api->CreateTensorWithDataAsOrtValue(memory_info, _state.data(), _state.size() * sizeof(float), state_node_dims, 3, ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT, &state_ort);
    if (status != nullptr) {
        const char* msg = ort_api->GetErrorMessage(status);
    ...
    status = ort_api->CreateTensorWithDataAsOrtValue(memory_info, &sr, sizeof(int64_t), nullptr, 0, ONNX_TENSOR_ELEMENT_DATA_TYPE_INT64, &sr_ort);
    if (status != nullptr) {
        const char* msg = ort_api->GetErrorMessage(status);
