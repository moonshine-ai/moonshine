...
int main(int , char **) {
    ...
    OrtStatus *env_status = ort_api->CreateEnv(ORT_LOGGING_LEVEL_WARNING, "OnnxruntimeTest", &ort_env);
    if (env_status != NULL) {   
    ...
    OrtStatus *session_options_status = ort_api->CreateSessionOptions(&session_options);
