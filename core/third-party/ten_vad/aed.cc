...
#define THROW_ON_ORT_ERROR(ort_api, expr)                                      \
  do {                                                                         \
    OrtStatus *status = (expr);                                                \
    if (status != NULL) {                                                      \
      const char *msg = ort_api->GetErrorMessage(status);                      \
      LOGF("ORT Error: %s at %s:%d", msg, __FILE__, __LINE__);                  \
...
AUP_MODULE_AIVAD::AUP_MODULE_AIVAD(unsigned char *onnx_data,
                                   size_t onnx_data_length) {
  ...
  THROW_ON_ORT_ERROR(ort_api, ort_api->CreateSessionFromArray(ort_env, onnx_data, onnx_data_length,
                                           session_options, &ort_session));
  ort_api->ReleaseSessionOptions(session_options);

  THROW_ON_ORT_ERROR(ort_api, ort_api->GetAllocatorWithDefaultOptions(&ort_allocator));
  size_t num_inputs;
  THROW_ON_ORT_ERROR(ort_api, ort_api->SessionGetInputCount(ort_session, &num_inputs));
  assert(num_inputs == AUP_AED_MODEL_IO_NUM);
  for (size_t i = 0; i < num_inputs; i++) {
    char *input_name;
    THROW_ON_ORT_ERROR(ort_api, ort_api->SessionGetInputName(ort_session, i, ort_allocator, &input_name));
    strncpy(input_names_buf[i], input_name, sizeof(input_names_buf[i]));
    input_names[i] = input_names_buf[i];
    THROW_ON_ORT_ERROR(ort_api, ort_api->AllocatorFree(ort_allocator, input_name));
  }

  size_t num_outputs;
  THROW_ON_ORT_ERROR(ort_api, ort_api->SessionGetOutputCount(ort_session, &num_outputs));
  assert(num_outputs == AUP_AED_MODEL_IO_NUM);
  for (size_t i = 0; i < num_outputs; i++) {
    char *output_name;
    THROW_ON_ORT_ERROR(ort_api, ort_api->SessionGetOutputName(ort_session, i, ort_allocator, &output_name));
    strncpy(output_names_buf[i], output_name, sizeof(output_names_buf[i]));
    output_names[i] = output_names_buf[i];
    THROW_ON_ORT_ERROR(ort_api, ort_api->AllocatorFree(ort_allocator, output_name));
  }
  ...
  status = ort_api->CreateCpuMemoryInfo(OrtDeviceAllocator, OrtMemTypeDefault,
                                        &memory_info);
  if (status != NULL) {
    LOGF("Failed to create memory info: %s",
            ort_api->GetErrorMessage(status));
    ort_api->ReleaseStatus(status);
  ...
  for (size_t i = 0; i < num_inputs; i++) {
    ...
    if (status != NULL) {
      LOGF("Failed to create input tensor %zu: %s", i,
              ort_api->GetErrorMessage(status));
      ort_api->ReleaseStatus(status);
      ort_api->ReleaseSession(ort_session);
      ort_api->ReleaseEnv(ort_env);
  ...
  for (size_t i = 0; i < num_outputs; i++) {
    ...
    if (status != NULL) {
      LOGF("Failed to create output tensor %zu: %s", i,
              ort_api->GetErrorMessage(status));
      ort_api->ReleaseStatus(status);
      ort_api->ReleaseSession(ort_session);
      ort_api->ReleaseEnv(ort_env);
...
int AUP_MODULE_AIVAD::Process(float *input, float *output) {
  ...
  THROW_ON_ORT_ERROR(ort_api, ort_api->Run(
      ort_session, NULL, input_names, ort_input_tensors, AUP_AED_MODEL_IO_NUM,
      output_names, AUP_AED_MODEL_IO_NUM, ort_output_tensors));
  float *output_data;
  THROW_ON_ORT_ERROR(ort_api, ort_api->GetTensorMutableData(ort_output_tensors[0], (void **)&output_data));
  *output = output_data[0];
  for (int i = 1; i < AUP_AED_MODEL_IO_NUM; i++) {
    THROW_ON_ORT_ERROR(ort_api, ort_api->GetTensorMutableData(ort_output_tensors[i], (void **)&output_data));
...
int AUP_Aed_memAllocate(void *stPtr, const Aed_StaticCfg *pCfg) {
  ...
  if (stHdl->aivadInf == NULL) {
    // stHdl->aivadInf = new
    // AUP_MODULE_AIVAD("../../../external-assets/ten-vad/ten-vad.onnx");
    stHdl->aivadInf = new AUP_MODULE_AIVAD(ten_vad_onnx_model_data,
                                           ten_vad_onnx_model_data_length);
    if (stHdl->aivadInf == NULL) {
      return -1;
    }
  }
