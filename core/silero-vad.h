...
private:
    ...
    static const int context_samples = 64;  // For 16kHz, 64 samples are added as context.
    ...
    // Initializes the common ONNX runtime environment (env, session_options, memory_info, allocator).
    void init_onnx_env();
...
public:
    SileroVad(int sample_rate = 16000, int windows_frame_size = 32,
    ...
    void predict(const std::vector<float>& data_chunk, float* out_probability, int* out_flag);
};
