...
def _clean_cut_index(wav: np.ndarray, sr: int, max_samples: int) -> int:
    ...
    A hard cut at exactly 4 s often lands mid-word, and ZipVoice then "continues" the prompt at the
    start of the generated speech (audible prompt leakage). Even cutting at a weak inter-word gap can
    leave the prompt ending on a dangling function word (e.g. "...slabs of"), which also leaks. So we
    ...
    thresh = 0.04 * peak
    silent = env < thresh
    min_keep = int(1.0 * sr)   # keep at least ~1 s of prompt speech
...
class _PromptTranscriber:
    ...
    ZipVoice needs the prompt transcript to match the prompt audio; since clips are trimmed to a fixed
    4 seconds, the original VCTK sentence no longer corresponds to what is actually spoken. Transcribing
    the trimmed clip (like ZipVoice's own auto-transcription) keeps them aligned and avoids prompt leakage.
    """
...
def main():
    ...
    parser.add_argument(
        ...
        default=True,
        help="Transcribe each trimmed clip with Moonshine ASR for an audio-matched prompt transcript "
    ...
    transcriber = None
    if args.transcribe:
        transcriber = _PromptTranscriber()
    ...
    for v in chosen:
        ...
        lines.append("};")
        lines.append("")
        entries.append((slug, v, gender, sym, len(pcm), prompt_text))

    lines.append("const ZipVoiceBuiltinVoice kZipVoiceBuiltinVoices[] = {")
    for slug, v, gender, sym, n, prompt_text in entries:
        lines.append("  {")
        ...
        lines.append(f'    "{c_escape(prompt_text)}",')
        lines.append(f"    {sym},")
        lines.append(f"    {n}u,")
    ...
    for slug, v, gender, _sym, n, prompt_text in entries:
        print(f"  {slug:<24} {v['accent']:<14} {gender:<7} {n} samples  \"{prompt_text[:48]}\"")
