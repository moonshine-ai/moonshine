...
public:
    ...
    bool Initialize() {
        ...
        hr = device_enumerator_->GetDefaultAudioEndpoint(eCapture, eConsole, &device_);
        if (FAILED(hr)) {
        ...
        if (hr == AUDCLNT_E_UNSUPPORTED_FORMAT) {
            ...
            hr = audio_client_->Initialize(AUDCLNT_SHAREMODE_SHARED,
                0, 0, 0, pwfx, nullptr);
            if (FAILED(hr)) {
                std::cerr << "Failed to initialize audio client with native format: " << std::hex << hr << std::endl;
                CoTaskMemFree(pwfx);
                return false;
            }
...
private:
    void CaptureLoop() {
        ...
        while (is_capturing_) {
            ...
            Sleep((DWORD)((1000.0 * buffer_frame_count_) / (2 * actual_sample_rate_)));

            hr = capture_client_->GetBuffer(&pData, &num_frames_available, &flags, nullptr, nullptr);
            if (FAILED(hr)) {
                continue;
            }
            ...
            if (actual_sample_rate_ == sample_rate_ && actual_channels_ == 1) {
                ...
                if (actual_sample_rate_ != sample_rate_) {
                    double ratio = static_cast<double>(sample_rate_) / actual_sample_rate_;
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if ((arg == "-m" || arg == "--model-path") && i + 1 < argc) {
            ...
            std::cout << "Usage: cli-transcriber [options]\n"
                      << "Options:\n"
                      << "  -m, --model-path PATH    Path to model directory (default: ../../../test-assets/tiny-en)\n"
    ...
    try {
        ...
        mic.SetAudioCallback([&transcriber](const std::vector<float>& audio_data, int32_t sample_rate) {
            try {
        ...
        // Note: Pressing Ctrl+C will terminate the process, and destructors will handle cleanup
        while (true) {
