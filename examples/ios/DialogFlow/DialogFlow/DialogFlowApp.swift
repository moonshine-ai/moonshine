...
/// SwiftUI can render.
@MainActor
final class DialogSession: ObservableObject {
    ...
    private var dialog: MoonshineVoice.DialogFlow? = nil
    ...
    func prepare() async {
        ...
        let flow = MoonshineVoice.DialogFlow()
    ...
    func startListening() {
        guard let dialog, isReady, !isListening else { return }
        do {
    ...
    func stopListening() {
        guard let dialog, isListening else { return }
        try? dialog.stopListening()
        isListening = false
        status = "Stopped."
    }
    ...
    func send(_ text: String) {
        guard let dialog, isReady else { return }
        let utterance = text.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !utterance.isEmpty else { return }
        Task { await dialog.handleUtterance(utterance) }
    }
...
@main
struct DialogFlowApp: App {
    @StateObject private var session = DialogSession()
