...
// Curated IPA for SpellingCNN letter readback homophones (spelling_labels.h).
// Checked before the baked dictionary and rule fallback so short vowel + solo
// fricative codas (e.g. "ess" for /ɛs/) keep an audible vowel in neural TTS.
bool LetterHomophoneToIpa(std::string_view word, std::string* ipa);
