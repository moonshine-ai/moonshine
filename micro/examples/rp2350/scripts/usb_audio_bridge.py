...
The RP2350 live-audio firmware (the ``moonshine_micro_echo`` or
``moonshine_micro_echo_wifi`` target) runs the streaming VAD + SpellingCNN
on-device and speaks the recognized letter/digit back via its formant TTS. This
script makes the laptop act as the board's microphone and speaker:
...
Example::
    ...
    python moonshine-micro/examples/rp2350/scripts/usb_audio_bridge.py
    python moonshine-micro/examples/rp2350/scripts/usb_audio_bridge.py \
    ...
    while True:
        ...
        if time.monotonic() > deadline:
            ...
            raise SystemExit(
                ...
                "board plugged in and running the live-audio firmware "
                "(not BOOTSEL)?")
        if not announced:
...
def main() -> int:
    ...
    if ready.wait(timeout=10.0):
        ...
        print("Device readiness banner not seen in 10 s; starting anyway "
              "(is it running the live-audio firmware?).")
