"""Generate the compiled-in data blobs for the cpp-tiny Pico build.
...
pairs of files under cpp-tiny/generated/:
...
Usage::
    ...
    python cpp-tiny/scripts/generate_embedded_data.py --clips-per-class 1
    python cpp-tiny/scripts/generate_embedded_data.py --max-classes 4
"""
...
# This script lives at cpp-tiny/stt/scripts/.
CPP_TINY_ROOT = pathlib.Path(__file__).resolve().parents[2]
REPO_ROOT = CPP_TINY_ROOT.parent
...
def _resolve_int8_mel_tflite() -> pathlib.Path:
    """Return the checked-in SpellingCNN int8 model under cpp-tiny/models/."""
    cand = CPP_TINY_ROOT / "models" / "spelling_cnn_letters_digits_mel_int8.tflite"
    if cand.is_file():
        return cand
    raise FileNotFoundError(
        f"No SpellingCNN int8 model found at {cand}. "
...
def _meta_sidecar(tflite: pathlib.Path) -> pathlib.Path:
    for name in (
        ...
        if cand.is_file():
            return cand
    return tflite.parent / "spelling_cnn_letters_digits_meta.json"
...
def _write_model_files(out_dir: pathlib.Path, tflite: pathlib.Path) -> None:
    ...
    header = textwrap.dedent(f"""\
        ...
        //   python cpp-tiny/scripts/generate_embedded_data.py
        //
        // Source: {tflite.relative_to(CPP_TINY_ROOT)}
        // Bytes:  {len(data)}
...
def _write_clips_files(
    out_dir: pathlib.Path,
    selected: list[tuple[int, str, pathlib.Path]],
    sample_rate: int,
    n_samples: int,
) -> None:
    ...
    header = textwrap.dedent(f"""\
        ...
        struct EmbeddedClip {{
          ...
          const char*   source_path;  // e.g. "real_spelled/captured/a/foo.wav"
    ...
    for i, (label_idx, label, wav_path) in enumerate(selected):
        samples = _load_clip(wav_path, sample_rate, n_samples)
        total_bytes += 2 * len(samples)
        parts.append(
            f"// [{i}] label={label!r}  src={wav_path.relative_to(REPO_ROOT)}\n"
    ...
    for i, (label_idx, label, wav_path) in enumerate(selected):
        rel = str(wav_path.relative_to(REPO_ROOT))
        parts.append(
            f'  {{ {label_idx:>3}, "{label}", "{rel}", '
...
def _pick_clips(
    ...
    for label_idx, label in enumerate(classes):
        if max_classes is not None and label_idx >= max_classes:
            break
        cls_dir = wavs_root / label
        if not cls_dir.is_dir():
            print(f"  WARNING: no clips for class {label!r} at {cls_dir}")
            continue
        wavs = sorted(cls_dir.glob("*.wav"))[:clips_per_class]
        if not wavs:
            print(f"  WARNING: empty class folder {cls_dir}")
            continue
        for w in wavs:
...
def main(argv: list[str]) -> int:
    ...
    ap.add_argument(
        "--wavs-dir", default="real_spelled/captured",
        help="Where to draw clips from (relative to repo root).",
    )
    ap.add_argument(
        "--out-dir", default=str(CPP_TINY_ROOT / "example-rp2350/generated"),
    ...
    if not out_dir.is_absolute():
        out_dir = CPP_TINY_ROOT / out_dir
    out_dir.mkdir(parents=True, exist_ok=True)

    wavs_root = REPO_ROOT / args.wavs_dir
    selected = _pick_clips(wavs_root, classes, args.clips_per_class, args.max_classes)
