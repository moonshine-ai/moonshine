 ...
 * splits the *SIMD* (ARM_MATH_DSP, SMLAD) GEMM across both M33 cores.
 *
 * Strategy:
 *   * Partition the output rows (rhs_rows == output channels) into two
 *     contiguous, even-aligned ranges. Each core writes a DISJOINT set
 *     of dst columns (stride rhs_rows); lhs/rhs are read-only shared.
 *     The math is bit-identical to single-core -- only the iteration
 *     order across output channels changes -- so outputs must match
 *     exactly (the on-device accuracy/per-clip-prob check is the test).
 *   * core 1 is launched ONCE into a persistent worker that blocks on
 *     the SIO FIFO between GEMMs (no per-call reset/relaunch -- that was
 *     the other thing that made the legacy path a net loss).
 *
 * Only the even rows are split; the lone odd row (if rhs_rows is odd)
 * is handled single-threaded by core 0 in the caller. Tiny GEMMs fall
 * back to single-core (the handshake isn't worth it).
 * ====================================================================== */
...
// Below this many output rows, the FIFO handshake costs more than the
// saved work, so just run single-core.
#ifndef SPELLING_TINY_MC_MIN_ROWS
#define SPELLING_TINY_MC_MIN_ROWS 8
#endif

// SIMD (SMLAD) GEMM over a contiguous, even-aligned subrange
// [row_start, row_end) of the output rows. `rhs` and `dst` are the
// ROW-0 base pointers; this function offsets to row_start internally.
// This is the verbatim ARM_MATH_DSP inner loop from arm_nn_mat_mult_nt_t_s8
// below, with the outer row pointers renamed (rhs->rhs_row, dst->dst_row)
// and the loop bounds parameterised. The odd-row tail is NOT handled here.
//
// __not_in_flash_func places this hot loop in SRAM. Both cores execute
// it during the parallel region; if it lived in flash they'd contend on
// the single XIP instruction-fetch path and erode the dual-core win
// (weight DATA is still read from flash, so this doesn't remove all
// contention -- but it removes the instruction half of it).
static void __not_in_flash_func(spelling_mm_dsp_rows)(int32_t row_start,
                                 int32_t row_end,
                                 const int8_t *lhs,
                                 const int8_t *rhs,
                                 const int32_t *bias,
                                 ...
                                 const int32_t lhs_cols_offset)
{
    ...
    const int8_t *rhs_row = rhs + (int32_t)row_start * rhs_cols;
    int8_t *dst_row = dst + row_start;

    for (int32_t rhs_rows_idx = row_start; rhs_rows_idx <= (row_end - 2); rhs_rows_idx += 2)
    {
        const int8_t *lhs_ptr = &lhs[0];
        int8_t *dst_ptr = &dst_row[0];
        ...
        for (int32_t x = 0; x < rhs_cols; ++x)
        {
            lhs_offset_contribution0 += rhs_row[x];
            lhs_offset_contribution1 += rhs_row[x + rhs_cols];
        }
        ...
        while (lhs_rows_idx)
        {
            const int8_t *rhs_ptr = &rhs_row[0];
...
// Generic one-shot dispatch to the persistent core-1 worker. Only one
// task runs at a time (TFLM is sequential on core 0), so a single
// function-pointer + arg pair is all the shared state we need. Ordering
// is provided by the SIO FIFO handshake plus the __dmb() barriers. This
// is the mechanism declared in spelling_multicore.h and reused by the
// 3x3 depthwise kernel.
static spelling_mc_task_fn g_spelling_mc_fn  = NULL;
static void               *g_spelling_mc_arg = NULL;
static bool                g_spelling_mc_started = false;

static void __not_in_flash_func(spelling_mc_core1_worker)(void)
{
    for (;;)
    {
        // Block (WFE inside the SDK) until core 0 hands us a task.
        (void)multicore_fifo_pop_blocking();
        __dmb();
        g_spelling_mc_fn(g_spelling_mc_arg);
        __dmb();
        multicore_fifo_push_blocking(0u);  // signal done
    }
}

void spelling_mc_dispatch_task(spelling_mc_task_fn fn, void *arg)
{
    if (!g_spelling_mc_started)
    {
        // Launch once into the persistent worker. After this returns the
        // FIFOs are drained and core 1 is parked in pop_blocking.
        multicore_launch_core1(spelling_mc_core1_worker);
        g_spelling_mc_started = true;
    }

    g_spelling_mc_fn  = fn;
    g_spelling_mc_arg = arg;

    __dmb();  // publish fn/arg before the go signal
    multicore_fifo_push_blocking(1u);
}

void spelling_mc_wait(void)
{
    (void)multicore_fifo_pop_blocking();  // block until core 1 done
    __dmb();  // observe core 1's writes before continuing
}

// GEMM-specific half: unpacked args + a task wrapper so the generic
// dispatch can run spelling_mm_dsp_rows() on core 1.
typedef struct {
    int32_t        row_start;
    int32_t        row_end;
    const int8_t  *lhs;
...
static void __not_in_flash_func(spelling_gemm_half_task)(void *arg)
{
    const spelling_gemm_half *a = (const spelling_gemm_half *)arg;
    spelling_mm_dsp_rows(a->row_start, a->row_end, a->lhs, a->rhs, a->bias, a->dst,
                         a->dst_multipliers, a->dst_shifts, a->lhs_rows, a->rhs_rows,
                         a->rhs_cols, a->lhs_offset, a->dst_offset,
                         a->activation_min, a->activation_max, a->lhs_cols_offset);
}
                                            ...
                                            const int32_t lhs_cols_offset)
{
    ...
    {
        // Split the even output rows across both M33 cores; the lone
        // odd row (if any) is done single-threaded below. Tiny GEMMs
        // skip the handshake and run wholly on core 0.
        const int32_t R_even = (rhs_rows / 2) * 2;
        if (rhs_rows >= SPELLING_TINY_MC_MIN_ROWS && R_even >= 4)
        {
            const int32_t num_pairs = R_even / 2;
            const int32_t pairs0    = num_pairs - (num_pairs / 2);  // core0 gets the ceil
            const int32_t mid       = pairs0 * 2;                   // even split point
            // core 1 takes [mid, R_even); core 0 takes [0, mid). `half`
            // lives on this stack frame, which stays valid until wait().
            spelling_gemm_half half = {mid, R_even, lhs, rhs, bias, dst,
                                       dst_multipliers, dst_shifts, lhs_rows, rhs_rows,
                                       rhs_cols, lhs_offset, dst_offset,
                                       activation_min, activation_max, lhs_cols_offset};
            spelling_mc_dispatch_task(spelling_gemm_half_task, &half);
            spelling_mm_dsp_rows(0, mid, lhs, rhs, bias, dst, dst_multipliers, dst_shifts,
                                 lhs_rows, rhs_rows, rhs_cols, lhs_offset, dst_offset,
                                 activation_min, activation_max, lhs_cols_offset);
            spelling_mc_wait();
        }
        else
        {
            spelling_mm_dsp_rows(0, R_even, lhs, rhs, bias, dst, dst_multipliers, dst_shifts,
                                 lhs_rows, rhs_rows, rhs_cols, lhs_offset, dst_offset,
                                 activation_min, activation_max, lhs_cols_offset);
        }

        // Odd-row tail (single output channel; scalar, one-shot).
        if (rhs_rows % 2)
        {
            const int8_t *lhs_ptr = &lhs[0];
            const int8_t *rhs_tail = rhs + (int32_t)R_even * rhs_cols;
            int8_t *dst_ptr = dst + R_even;
            for (int32_t lhs_rows_idx = 0; lhs_rows_idx < lhs_rows; ++lhs_rows_idx)
            {
                const int8_t *rhs_ptr = rhs_tail;
                int32_t res00 = 0;
                if (bias)
                {
...
#else  // !SPELLING_TINY_MULTICORE: original single-core DSP path
    const int32_t rhs_off0 = rhs_cols - 4;
    const int32_t lhs_off0 = lhs_cols_offset - 4;
