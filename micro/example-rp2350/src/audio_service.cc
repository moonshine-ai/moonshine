...
// spoken). Keep in sync with the host bridge, which resumes listening on a
// sub-threshold RESULT instead of waiting for a spoken reply.
...
void RunAudioService(AudioInput& input, AudioOutput& output, uint8_t* arena,
  ...
  printf("[audio] service entered (arena=%u, WS=%d, HOP=%d)\n",
         static_cast<unsigned>(arena_size), WS, HOP);
  fflush(stdout);

  // Persistent front-end (outside the arena): the streaming VAD mel front-end
  // (shares the FFT + STT Hann window), the moving-average segmenter, and the
  // STT log-mel. The streamer holds a multi-KB log-mel ring; keeping it
  // `static` (BSS, not the stack) prevents PushHop's FFT scratch and TFLM's
  // deep Invoke() frames from corrupting the ring on the small (4 KB) core
  // stack.
  static spelling::MelStreamer streamer(
      spelling::kVadNMels, spelling::kVadWindowFrames, spelling::kVadNFft,
      spelling::kMelWindow, spelling::kVadMelNzOff, spelling::kVadMelNzIdx,
  ...
  // Static (BSS, not stack) for the same reason as `streamer` above.
  static spelling::LogMelSpectrogram stt_log_mel(slm);
  ...
  for (;;) {
    ...
    std::size_t seg_start = 0, seg_end = 0;
    {
      ...
      while (!got_clip) {
        ...
        streamer.PushHop(&window[WS - HOP]);
    ...
    float prob = 0.0f;
    {
      ...
      stt_log_mel.Compute(window, static_cast<std::size_t>(WS), feats);
    ...
    // don't speak them. Skip the mute tail too since there's no reply to echo.
    ...
    // ---- TTS phase (StreamSynth uses the arena as working memory) ----
    {
      ...
      if (synth.BeginText(SpokenForLabel(spelling::kClassLabels[pred]), opts) ==
          tts::kStreamOk) {
    ...
    }  // synth destroyed; loop back and rebuild Vad.
    ...
    // speech detection. VAD state is rebuilt fresh on the next listening pass.
