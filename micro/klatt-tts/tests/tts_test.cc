...
#include "g2p/g2p.h"  // shared G2P front end (TextToPhones)
...
TF_LITE_MICRO_TEST(G2PProducesPhones) {
  const std::vector<std::string> toks = g2p::TextToPhones("hello world");
...
TF_LITE_MICRO_TEST(G2PNumberNormalization) {
  ...
  const std::vector<std::string> toks = g2p::TextToPhones("21");
