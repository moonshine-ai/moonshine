...
// This runs entirely in the core's built-in voice-activity detector, so no
// model download is involved and the tests always run.
...
test('the requested clip length is honoured', { skip: speechSkip }, () => {
  const { audio, sampleRate } = readWav(speechPath);
  const clip = mod.extractSpeechClip(audio, sampleRate, 2, 1);
  assert.equal(clip.isComplete, true);
  assert.equal(clip.audio.length, 2 * 16000);
});

test('extractSpeechClip falls back rather than failing on silence', { skip }, async () => {
  const { extractSpeechClip } = await importApi();
  const silence = new Float32Array(16000 * 6);
  const clip = await extractSpeechClip(silence, 16000, { module: mod });
...
test('VoiceClone reports readiness as audio is fed in', { skip: speechSkip }, async () => {
  ...
  const { audio, sampleRate } = readWav(speechPath);
  const clone = new VoiceClone(mod);
