...
public class Transcriber {
    ...
    public void loadFromFiles(String modelRootDir, int modelArch) {
        ...
        this.transcriberHandle = JNI.moonshineLoadTranscriberFromFiles(modelRootDir, modelArch,
                options.toArray(new TranscriberOption[0]));
        if (this.transcriberHandle < 0) {
            throw new RuntimeException("Failed to load transcriber from files: '" + modelRootDir + "'");
        }
        this.getDefaultStreamHandle();
    }

    public void loadFromMemory(byte[] encoderModelData, byte[] decoderModelData, byte[] tokenizerData, int modelArch) {
        JNI.ensureLibraryLoaded();
        this.transcriberHandle = JNI.moonshineLoadTranscriberFromMemory(encoderModelData, decoderModelData,
                tokenizerData, modelArch, options.toArray(new TranscriberOption[0]));
        if (this.transcriberHandle < 0) {
    ...
    protected void finalize() throws Throwable {
        if (this.transcriberHandle >= 0) {
            if (this.defaultStreamHandle >= 0) {
                JNI.moonshineFreeStream(this.transcriberHandle, this.defaultStreamHandle);
    ...
    public Transcript transcribeWithoutStreaming(float[] audioData, int sampleRate) {
        return JNI.moonshineTranscribeWithoutStreaming(this.transcriberHandle, audioData, sampleRate, 0);
    }
    ...
    public void stopStream(int streamHandle) {
        ...
        Transcript transcript = JNI.moonshineTranscribeStream(this.transcriberHandle, streamHandle, 0);
        this.notifyFromTranscript(transcript, streamHandle);
    }

    public void start() {
        this.startStream(this.getDefaultStreamHandle());
    }

    public void stop() {
        this.stopStream(this.getDefaultStreamHandle());
    }
    ...
    public void addAudioToStream(int streamHandle, float[] audioData, int sampleRate) {
        JNI.moonshineAddAudioToStream(this.transcriberHandle, streamHandle, audioData, sampleRate);
        Transcript transcript = JNI.moonshineTranscribeStream(this.transcriberHandle, streamHandle, 0);
        if (transcript == null) {
            throw new RuntimeException("Failed to transcribe stream: " + streamHandle);
        }
        this.notifyFromTranscript(transcript, streamHandle);
    }
